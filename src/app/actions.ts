...
import { collection, addDoc, serverTimestamp, query, where, getDocs, orderBy, limit, doc, getDoc, Timestamp } from "firebase/firestore";
...
export async function getStrategicRecommendationsAction(risks: RiskIndicator[], province: string) {
    "use server";
    try {
        const recommendation = await generateStrategicRecommendations(risks, province);
...
export async function performDeepResearchAction(communityId: string, communityName: string) {
    "use server";

    try {
        // 1. Tavily Search
        const searchResults = await searchCommunityData(`${communityName} infrastructure news sentiment 2024`);
...
export async function triggerForecastingAction(communityId: string, communityName: string, category: string) {
    "use server";

    try {
        // Fetch recent signals for context
        const q = query(collection(db, "community_signals"), where("communityId", "==", communityId));
        const snapshot = await getDocs(q);
        const history = snapshot.docs.map(d => d.data().content).join("\n");

        const forecast = await forecastInfraDemand(communityName, category, history);
...
export async function getExecutiveBriefing() {
    "use server";
    try {
        const q = query(collection(db, "community_signals"), orderBy("timestamp", "desc"), limit(20));
        const snapshot = await getDocs(q);
...
export async function getAdvancedInsights() {
    "use server";
    try {
        const q = query(collection(db, "community_signals"), orderBy("timestamp", "desc"));
        const snapshot = await getDocs(q);
        ...
        if (signals.length === 0) {
            return {
                success: true,
                data: { proportionality: [], velocity: 0, reliability: 85, totalSignals: 0 }
            };
        }
        ...
        const getAvgSentiment = (arr: AISignal[]) => {
            ...
            const scores = arr.map(s => s.sentiment === 'positive' ? 90 : s.sentiment === 'negative' ? 10 : 50);
            return scores.reduce((a, b) => a + b, 0) / arr.length;
        };

        const velocity = getAvgSentiment(recent as AISignal[]) - getAvgSentiment(older as AISignal[]);

        // 3. Influencer/Source Reliability (Signals grounded in News vs Social)
        const newsGrounded = signals.filter((s: any) => s.source && (s.source.includes('gov.za') || s.source.includes('news24'))).length;
...
export async function getStrategicInsights(serviceId: string = "apex") {
    ...
    try {
        ...
        return { success: false, error: `No strategic insights found for ${serviceId}` };
    } catch (error) {
        return { success: false, error: String(error) };
    }
}
...
export async function getVisualAnalytics(serviceId: string = "apex") {
    ...
    try {
        ...
        return { success: false, error: `No visual analytics found for ${serviceId}` };
    } catch (error) {
        return { success: false, error: String(error) };
    }
}
...
export async function getLogicalIntelligence(serviceId: string, province: string, municipality: string) {
    "use server";
    try {
        const docId = `${serviceId}_${province}_${municipality}`.replace(/\s+/g, '_');
        ...
        return { success: false, error: `No logical intelligence found for ${docId}` };
    } catch (error) {
        return { success: false, error: String(error) };
    }
}
...
export async function getGroundedSignals(communityId: string, category: string) {
    "use server";
    try {
        const signalsRef = collection(db, "community_signals");
        const q = query(
