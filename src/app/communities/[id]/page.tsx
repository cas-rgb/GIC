      ...
      <div className="min-h-screen flex items-center justify-center bg-[#0B0F17] text-white">
        <div className="text-center space-y-4">
          <p className="text-white/40">
            Community intelligence node not found.
          </p>
          <button
            ...
            <div className="flex items-center gap-4 text-white/60">
              <div className="flex items-center gap-1.5 bg-white/5 px-3 py-1 rounded-full border border-white/5">
                ...
                <span className="text-xs font-medium">
                  {community.province}, South Africa
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="max-w-[1600px] mx-auto px-12 grid grid-cols-12 gap-8 -mt-8">
        ...
        <div className="col-span-12 grid grid-cols-1 md:grid-cols-4 gap-6 z-30">
          ...
          <MetricCard
            ...
            value={`${(community.riskScore / 10).toFixed(1)}/10`}
            detail="CRITICAL"
            color="rose"
          />
          <MetricCard
            label="Infra Index"
            value={`${community.infrastructureScore}%`}
            trend="-2.1%"
            isUp={false}
            color="blue"
          />
        ...
        <div className="col-span-12 lg:col-span-8 space-y-8">
          ...
          <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 space-y-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/40">
                    Real-time infrastructure pulse tracking
                  </p>
                </div>
              </div>
              <div className="flex bg-white/5 p-1 rounded-xl border border-white/10">
                {["1D", "1W", "1M"].map((t) => (
                  <button
                    key={t}
                    className={`px-4 py-1.5 text-[10px] font-bold rounded-lg transition-all ${t === "1W" ? "bg-primary text-white" : "text-white/40 hover:text-white"}`}
                  >
                    {t}
                  </button>
                ))}
            ...
            <p className="text-xs text-white/30 italic">
              * Activity patterns indicate a significant spike in
              infrastructure-related complaints on Saturday, correlating with
              the scheduled maintenance cycle in Sector 4.
            </p>
          </div>
        ...
        <div className="col-span-12 lg:col-span-4 space-y-8">
          ...
          <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 space-y-8 h-fit">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-4">
                <IntelligenceSnippet
                  text="Improved lighting in the North Park area has significantly boosted nighttime foot traffic."
                  source="Field Report #8821"
                />
                <IntelligenceSnippet
                  text="Ongoing power outages in the Southern Industrial sector are creating friction."
                  source="Digital Signal Scraping"
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white/5 border border-white/10 rounded-[2.5rem] p-8 space-y-6">
            <div className="flex items-center justify-between">
              ...
              <span className="px-2 py-0.5 bg-rose-500/20 text-rose-500 rounded-full text-[10px] font-bold">
                3 ACTIVE
              </span>
            </div>
            <div className="space-y-4">
              <AlertItem
                type="POWER GRID"
                title="Substation 4B Fault"
                time="14m ago"
                description="Cascading failure suspected. Dispatching unit."
              />
              <AlertItem
                type="TRAFFIC"
                title="Unscheduled Road Blockage"
                time="1h ago"
                description="A104 Bypass blocked due to localized protest action."
              />
            </div>
          </div>
        </div>
  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-4">
        ...
        {trend && (
          <span
            className={`text-[10px] font-bold flex items-center gap-1 ${isUp ? "text-emerald-400" : "text-rose-400"}`}
          >
            {isUp ? (
      ...
      <p
        className={`text-4xl font-black tracking-tighter ${color === "rose" ? "text-rose-400" : color === "blue" ? "text-blue-400" : "text-white"}`}
      >
        {value}
      </p>
    </motion.div>
  ...
  return (
    <div className="bg-white/5 border border-white/10 rounded-3xl p-6 flex flex-col gap-4">
      <div className="flex justify-between items-start">
        ...
        <span
          className={`text-[10px] font-bold ${trend.startsWith("+") ? "text-emerald-400" : trend === "0.0" ? "text-white/20" : "text-rose-400"}`}
        >
...
function IntelligenceSnippet({ text, source }: any) {
  return (
    <div className="space-y-1">
      <p className="text-xs text-white/70 leading-relaxed italic border-l-2 border-primary/30 pl-3">
        "{text}"
      </p>
      <p className="text-[9px] font-bold text-white/20 uppercase tracking-widest pl-3">
        — {source}
      </p>
    </div>
  );
