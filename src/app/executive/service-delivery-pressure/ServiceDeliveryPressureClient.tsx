...
export default function ServiceDeliveryPressureClient() {
  ...
  const fetchData = async () => {
    ...
    try {
      const result =
        await DashboardTwoAggregator.getServicePressureData(province);
      setData(result);
    } catch (err) {
        ...
        <div className="max-w-[1600px] mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="space-y-1">
            ...
            <p className="text-slate-500 font-medium text-sm max-w-2xl">
              Cross-sector friction analysis, community stress acceleration, and
              municipality exposure rankings.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div>
            <GovernedChart
              ...
              metric={kpis.topDomain}
              option={configs.getPressureBreakdownConfig(
                visuals.breakdown || [],
              )}
              height="500px"
            />
          ...
          <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center">
            <div className="space-y-1">
              <h2 className="text-sm font-black text-slate-900 uppercase tracking-widest flex items-center gap-2">
                <Database className="w-4 h-4 text-rose-500" /> Operational Risk
                Ground Truth
              </h2>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                Live Signals classified as High Pressure or Escalating
              </p>
            </div>
