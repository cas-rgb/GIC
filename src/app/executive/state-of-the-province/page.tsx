...
import { GovernedChart, getStackedBarOption, getTrendOption, getRankedMuniOption, getScatterOption } from '@/components/analytics/GovernedCharts';
...
export default function StateOfTheProvincePage() {
    ...
    const loadData = async (prov: string) => {
        setLoading(true);
        try {
            const [kpis, breakdown, trend, distribution, matrix, signals] = await Promise.all([
    ...
    if (!data && loading) {
        return <div className="p-12 text-slate-400 font-black animate-pulse uppercase tracking-[0.2em]">Initialising Intelligence Layer...</div>;
    }
    ...
    return (
        <div className="min-h-screen bg-slate-50 text-slate-900 p-8 pb-24">
            ...
            <header className="flex justify-between items-start mb-12">
                <div>
                    <h1 className="text-4xl font-black uppercase tracking-tighter mb-2 italic">State of the Province</h1>
            ...
            <div className="grid grid-cols-4 gap-6 mb-12">
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <GovernedValue metric={data?.kpis?.concernVolume} showTrace />
                </div>
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <GovernedValue metric={data?.kpis?.topConcern} valueClassName="text-xl" />
                </div>
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <GovernedValue metric={data?.kpis?.concernVelocity} />
                </div>
                <div className="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm">
                    <GovernedValue metric={data?.kpis?.alignmentScore} valueClassName="text-2xl" />
                </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-8 mb-8">
                <GovernedChart 
                    title="Public Concern Topic Breakdown"
                    metric={{
                        id: 'visual-2',
                        label: 'Topic Breakdown',
                        value: data?.breakdown?.length || null,
                        confidence: 0.8,
                        rating: data?.breakdown?.length > 0 ? 'HIGH' : 'INSUFFICIENT',
                        trace: [{ table: 'community_issue', query: 'aggregate', sourceCount: data?.breakdown?.length || 0, timestamp: new Date().toISOString() }]
                    }}
                    option={getStackedBarOption(data?.breakdown || [])}
                />
                <GovernedChart 
            ...
            <div className="mt-12">
                <div className="flex items-center justify-between mb-6">
                    <div>
                        <h2 className="text-xl font-black uppercase tracking-tight italic">Signal Intelligence Feed</h2>
                        <p className="text-[10px] text-slate-500 font-bold uppercase tracking-[0.2em]">Verified Recent Signals / Ground Truth Traceability</p>
                    </div>
                </div>
                <div className="grid grid-cols-3 gap-8">
                    <div className="col-span-2">
                        <SignalIntelligenceFeed signals={data?.signals || []} loading={loading} />
                    </div>
                    <div className="flex flex-col gap-6">
                        <div className="bg-slate-900 rounded-3xl p-8 text-white relative overflow-hidden">
                            <div className="z-10 relative">
                                <h3 className="text-sm font-black uppercase tracking-widest mb-4">Intelligence Note</h3>
                                <p className="text-xs text-slate-300 leading-relaxed font-bold italic opacity-80">
                                    "The signals displayed here represent recent detections classified by our strategic intelligence layer. Every signal is traced back to a verified regional source."
                                </p>
                            </div>
                        ...
                        <div className="bg-white border border-slate-100 rounded-3xl p-8">
                            <h3 className="text-xs font-black uppercase tracking-widest mb-6 text-slate-900 border-b pb-4">Signal Integrity</h3>
                            <div className="space-y-4">
                                ...
                                <div className="flex justify-between items-center text-[10px] font-bold">
                                    <span className="text-slate-500 uppercase tracking-wider">Source Parity</span>
