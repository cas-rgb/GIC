...
export default function CommunityProfile({
  ...
  useEffect(() => {
    async function loadProfile() {
      const res = await (
        await import("@/app/intel-actions")
      ).getCommunityIntelligence();
      if (res.success && res.communities) {
        const found =
          res.communities.find((c: any) => c.id === params.id) ||
          res.communities[0];
        setCommunity(found);
      }
    }
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-4">
            ...
            >
              <FileSearch
                className={`w-4 h-4 text-gic-blue ${isGenerating ? "animate-pulse" : ""}`}
              />
          ...
          <div className="col-span-4 space-y-12">
            ...
            >
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-10">
                  <div>
                    <p className="text-[11px] font-black text-slate-600 uppercase tracking-[0.4em] mb-2">
                      Regional Fragility
                    </p>
            ...
            <div className="gic-glass-apex p-10 rounded-[3.5rem] border border-white/[0.06] shadow-3xl">
              <div className="flex items-center justify-between mb-10 px-2">
                ...
                >
                  <RefreshCw
                    className={`w-4 h-4 text-slate-500 group-hover:text-blue-400 ${isPullingSignals ? "animate-spin" : ""}`}
                  />
                </button>
              </div>
              <div className="space-y-6 max-h-[400px] overflow-y-auto pr-2 scrollbar-hide">
                {realSignals.length > 0
                  ? realSignals.map((signal, i) => (
                      ...
                      >
                        <p className="text-[15px] font-bold text-white group-hover:text-blue-400 transition-colors leading-tight tracking-tight">
                          {signal.text}
                        </p>
                  ...
                  : [
                      {
                        title: `Water Supply Disruptions in ${communityName}`,
                        source: "EWN News",
                        time: "2h ago",
                      },
                      {
                        title: `Residents Demand Road Maintenance in ${community.municipality}`,
                        source: "Local Gazette",
                        time: "5h ago",
                      },
                      {
                        title: "Infrastructure Audit Underway",
                        source: "GIC Tracker",
                        time: "1d ago",
                      },
                    ].map((signal, i) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-4 mt-4 text-[11px] text-slate-600 font-black uppercase tracking-tighter">
                          <span className="text-blue-500">{signal.source}</span>
                          <span className="flex items-center gap-2">
                            <Clock className="w-3.5 h-3.5" /> {signal.time}
                          </span>
                        </div>
                      </motion.div>
                    ))}
                ...
                <AnimatePresence mode="wait">
                  {activeTab === "audit" && dossier ? (
                      ...
                      <div className="grid grid-cols-2 gap-16">
                        <div className="space-y-10">
                          ...
                          <div className="space-y-6">
                            {dossier.strategicRisks.map(
                              (risk: string, i: number) => (
                                <div
                                  key={i}
                                  className="p-8 bg-white/[0.01] border border-white/[0.05] rounded-[2rem] text-[14px] font-bold text-slate-500 flex gap-6 hover:border-blue-500/30 transition-all duration-1000 group/item"
                                >
                        ...
                        <div className="space-y-10">
                          ...
                          <div className="space-y-6">
                            {dossier.growthOpportunities.map(
                              (opp: string, i: number) => (
                                <div
                                  key={i}
                                  className="p-8 bg-white/[0.01] border border-white/[0.05] rounded-[2rem] text-[14px] font-bold text-slate-500 flex gap-6 hover:border-blue-500/30 transition-all duration-1000 group/item"
                      ...
                      <table className="w-full text-left">
                        <thead>
                          <tr className="text-[11px] font-black text-slate-700 uppercase tracking-[0.4em]">
                            ...
                            <th className="pb-10 text-center">
                              Integrity Vector
                            </th>
                        ...
                        <tbody className="text-[15px] font-bold text-slate-400">
                          {[
                            {
                              name: "Sizwe Mdluli",
                              topic: "Water Rights Corridors",
                              reach: "12,400",
                              score: 94,
                            },
                            {
                              name: "Soweto Civic Organisation",
                              topic: "Structural Maintenance",
                              reach: "45,201",
                              score: 88,
                            },
                            {
                            ...
                            >
                              <td className="py-8 pl-6 font-black transition-all text-white group-hover:text-blue-400 group-hover:translate-x-2">
                                {inf.name}
                              </td>
                              <td className="py-8 text-slate-600 italic font-medium">
                                {inf.topic}
                              </td>
                              <td className="py-8 text-center text-slate-500 font-black tracking-widest">
                                {inf.reach}
                              </td>
                              <td className="py-8 text-center">
                                <span className="bg-blue-500/10 text-blue-500 px-4 py-1.5 rounded-xl border border-blue-500/20 font-black tracking-[0.2em] text-[10px]">
                                  {inf.score} / 100
                                </span>
                              </td>
                    ...
                    >
                      ...
                      <p className="font-black uppercase text-[12px] tracking-[0.5em] mb-4 text-white">
                        Synthesizing Regional Data Streams
                      </p>
                      <p className="text-sm text-slate-600 max-w-[340px] leading-relaxed font-medium">
                        Processing multi-channel telemetry and community signals
                        for high-fidelity regional biopsy.
                      </p>
                      {!dossier && (
                        <button
                          onClick={handleGenerateAudit}
