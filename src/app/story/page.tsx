  ...
  useEffect(() => {
    async function loadStory() {
      ...
      if (sigRes.success && sigRes.signals && sigRes.signals.length > 0) {
        const sorted = [...sigRes.signals].sort(
          (a: any, b: any) =>
            (b.urgencyValue || b.urgency || 0.5) -
            (a.urgencyValue || a.urgency || 0.5),
        );
      ...
      if (metaRes.success && metaRes.municipalities) {
        setHotspots(
          metaRes.municipalities
            .slice(0, 5)
            .map((m: string) => ({ label: m, value: m.toLowerCase() })),
        );
      }
      setIsLoading(false);
    }
          ...
          filters={[
            {
              label: "Hotspot Node",
              icon: <Map />,
              options: [{ label: "All Hotspots", value: "all" }, ...hotspots],
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-[10px] font-black text-rose-400 uppercase tracking-[0.3em]">
                  {isLoading
                    ? "Scanning Telemetry..."
                    : `Event Alert: ${displayStory.eventTrigger || "Strategic Signal"}`}
                </span>
              </div>

              <h2 className="text-3xl font-display font-bold text-white leading-[1.1] mb-5 tracking-tight">
                {isLoading
                  ? "System Discovery Active"
                  : displayStory.text.length > 50
                    ? displayStory.text.substring(0, 50) + "..."
                    : displayStory.text}
              </h2>
              ...
              <p className="text-[12px] text-slate-400 font-medium leading-relaxed mb-10 opacity-80">
                {isLoading
                  ? "Synthesizing clinical signals from global internet telemetry and regional sensors..."
                  : displayStory.text}
              </p>
              ...
              <div className="flex flex-col gap-4">
                <Indicator
                  label="01 Issue"
                  value={
                    displayStory.layers?.includes("issue")
                      ? "Clinical Alert"
                      : "Nominal"
                  }
                  color="text-rose-400"
                ...
                <Indicator
                  label="03 Momentum"
                  value={`Velocity [${displayStory.momentum?.toFixed(2) || "0.00"}]`}
                  color="text-emerald-400"
                  icon={<Zap className="w-3.5 h-3.5" />}
                />
                <Indicator
                  label="07 Event"
                  value={displayStory.eventTrigger || "None"}
                  color="text-rose-500"
            ...
            <div className="flex flex-col gap-6 items-end pointer-events-auto">
              ...
              <div className="bg-slate-900/90 backdrop-blur-3xl border border-white/10 p-5 rounded-[2rem] flex flex-col gap-5 items-center">
                ...
                <div className="flex flex-col items-center gap-1">
                  ...
                  <span className="text-[11px] font-mono font-bold text-gic-blue">
                    ST-0922-X
                  </span>
                </div>
              </div>
              ...
              <div className="relative flex items-center justify-between">
                <div className="flex gap-16 items-center">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p className="text-base font-bold text-white tracking-tight">
                        {displayStory.momentum
                          ? (displayStory.momentum * 12).toFixed(1)
                          : "4.2"}
                        km Tactical Zone
                      </p>
                    </div>
                  ...
                  <div>
                    ...
                    <p className="text-base font-bold text-white tracking-tight">
                      {displayStory.province},{" "}
                      {displayStory.municipality || "Core Grid"}
                    </p>
                  </div>
                  <div className="w-px h-10 bg-white/5" />
                  <div>
                    <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-1">
                      Fiscal Exposure Index
                    </p>
                    <div className="flex items-center gap-2">
                      <span className="text-base font-bold text-rose-400">
                        R
                        {(displayStory.urgency
                          ? displayStory.urgency * 25
                          : 120
                        ).toFixed(0)}
                        M
                      </span>
                      <span className="text-[10px] font-bold text-rose-500/50 uppercase tracking-tighter">
                        at Risk
                      </span>
                    </div>
                  </div>
    ...
    <div className="flex items-center justify-between p-4 bg-white/[0.05] border border-white/10 rounded-2xl group/indicator hover:bg-white/[0.08] hover:border-white/20 transition-all duration-500 shadow-lg">
      <div className="flex items-center gap-4">
        <div className="p-2.5 bg-slate-950 rounded-xl group-hover/indicator:scale-110 group-hover/indicator:text-white transition-all duration-500 border border-white/5 shadow-inner">
          <div className="w-3.5 h-3.5 opacity-70 group-hover:opacity-100">
            {icon}
          </div>
        </div>
        <span className="text-[11px] font-black text-slate-400 uppercase tracking-[0.15em] group-hover:text-slate-200 transition-colors">
          {label}
        </span>
      </div>
      <span
        className={`text-[11px] font-black uppercase tracking-wider px-3 py-1 bg-black/20 rounded-lg border border-white/5 ${color}`}
      >
        {value}
      </span>
    </div>
