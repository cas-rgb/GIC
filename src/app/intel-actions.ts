...
import { SignalIngestionService, GICServiceLine, TimeWindow } from "@/services/signal-ingestion";
 ...
 */
export async function discoverRealSignals(context: string = "Gauteng Infrastructure") {
    try {
...
export async function extractEvidenceSnippet(url: string) {
    try {
        ...
        return { success: true, snippet: "Extracted clinical snippet placeholder." };
    } catch (error) {
        return { success: false, error: String(error) };
    }
}
...
import { SA_REGIONAL_REGISTRY, SADC_REGIONAL_REGISTRY } from "@/data/regional-registry";
...
export async function ingestDashboardTwoData(provinces: string[] = ["Gauteng", "Western Cape"]) {
    try {
 ...
 */
export async function orchestrateGrandIngestion(provinceName: string, country: 'South Africa' | 'Namibia' | 'Botswana' | 'Eswatini' | 'Lesotho' = 'South Africa') {
    try {
        let province;
        if (country === 'South Africa') {
            province = SA_REGIONAL_REGISTRY.provinces.find(p => p.name === provinceName);
        } else {
            province = SADC_REGIONAL_REGISTRY[country]?.provinces.find(p => p.name === provinceName);
        }

        if (!province) throw new Error(`${country} Province ${provinceName} not found`);

        const summary = [];
        for (const muni of province.municipalities) {
            console.log(`Orchestrating deep dive for ${muni.name} in ${country}...`);
            const muniResults = await GlobalDataOrchestrator.performRegionalDeepDive(country, province.name, muni.name);
            ...
            if (muni.wards && muni.wards.length > 0) {
                for (const ward of muni.wards) {
                    ...
                    const wardResults = await GlobalDataOrchestrator.performRegionalDeepDive(country, province.name, muni.name, ward);
                    summary.push({ municipality: `${muni.name} (${ward})`, count: wardResults.length });
                }
            }
        }
...
export async function getRegionalIntelligence(params: {
    ...
    try {
        const { country = 'South Africa', province = 'Gauteng', municipality, ward, domain, sourceType, sector } = params;
        const filters = [
            where('country', '==', country)
        ];

        if (province && province !== 'All Provinces') filters.push(where('province', '==', province));
        if (municipality && municipality !== 'All Municipalities') filters.push(where('municipality', '==', municipality));
        if (ward && ward !== 'All Wards') filters.push(where('ward', '==', ward));

        const [tendersRes, leadersRes, planningRes, risksRes, communitiesRes, newsRes] = await Promise.all([
        ...
        const leaders = leadersRes.docs.map(doc => ({ id: doc.id, ...doc.data() as any }));
        const planning = planningRes.docs.map(doc => ({ 
            id: doc.id, 
            ...doc.data() as any,
            budget: doc.data().budgetAllocation || doc.data().budget || 0 
        }));
        const risks = risksRes.docs.map(doc => ({ id: doc.id, ...doc.data() as any }));
        const communities = communitiesRes.docs.map(doc => ({ id: doc.id, ...doc.data() as any }));
        const news = newsRes.docs.map(doc => ({ id: doc.id, ...doc.data() as any }));
...
export async function getRegionalSOS(province: string, country: string = 'South Africa') {
    try {
...
import { GovernedMetric, METRIC_REGISTRY, getConfidenceRating } from "@/lib/reporting-schema";
...
export async function getExecutiveNationalMetrics() {
    ...
    try {
        ...
        const [allCommunities, allTenders, allRisks, allPlanning] = await Promise.all([
        ...
        const results = countries.map((country) => {
            const countryRisks = allRisks.docs.filter(d => d.data().country === country);
            const countryTenders = allTenders.docs.filter(d => d.data().country === country);

            const avgRisk = countryRisks.length > 0 
                ? countryRisks.reduce((acc, d) => acc + (d.data().severityValue || d.data().severity || 5), 0) / countryRisks.length
                : 0;

            const confidence = Math.min((countryRisks.length + countryTenders.length) / 10, 1.0);
        ...
        const nationalConfidenceVal = results.reduce((acc, r) => acc + r.confidence, 0) / countries.length;
        ...
        const nationalConfidence: GovernedMetric<number> = {
            ...
            trace: [
                { table: 'riskSignals', query: 'ALL_COUNTRIES', sourceCount: allRisks.size, timestamp },
                { table: 'gic_projects', query: 'ALL_COUNTRIES', sourceCount: allTenders.size, timestamp }
            ]
        };
        ...
        return {
            success: true,
            countries: results,
            topRisks: allRisks.docs.slice(0, 10).map(d => ({ id: d.id, ...d.data() })),
            communities: allCommunities.docs.map(d => ({ id: d.id, ...d.data() })),
            planning: allPlanning.docs.map(d => ({ id: d.id, ...d.data() })),
            nationalConfidence,
            deliveryVelocity: nationalConfidenceVal > 0.8 ? 'ACCELERATING' : 'Awaiting source integration'
        };
    } catch (error) {
...
export async function getProvinceIntelligence(province: string, allowIngest: boolean = false) {
    const timestamp = new Date().toISOString();
    try {
        // 1. Fetch Primary Signals (Public Concerns)
        let signalsSnap = await getDocs(query(
        ...
        if (signalsSnap.empty && allowIngest) {
            console.log(`Governed Grounding: No signals found for ${province}. Triggering ingestion...`);
            ...
            // Refresh snapshot
            signalsSnap = await getDocs(query(
                collection(db, 'riskSignals'),
                where('province', '==', province)
            ));
        }
        ...
        const topTopicShare = topTopicEntry ? (topTopicEntry[1] / (totalMentions || 1)) * 100 : 0;
        ...
        const mentionsToday = signals.filter(s => s.createdAt >= startOfToday).length;
        const mentionsLast7Days = signals.filter(s => s.createdAt >= sevenDaysAgo && s.createdAt < startOfToday).length;
        const avgLast7Days = mentionsLast7Days / 7;
        
        const velocityValue = avgLast7Days > 0 ? mentionsToday / avgLast7Days : (mentionsToday > 0 ? 1.5 : 1.0);
        ...
        const finalAlignmentScore = alignmentTopicsCount > 0 ? (alignmentScoreTotal / alignmentTopicsCount) * 100 : null;
        ...
        const calcConfidence = (count: number, expected: number) => Math.min(count / expected, 1.0);
        ...
        const metrics = {
            volume: {
                ...
                trace: [{ table: 'riskSignals', query: `province=${province}`, sourceCount: totalMentions, timestamp }]
            ...
            topConcern: {
                ...
                trace: [{ table: 'riskSignals', query: `max(topic_frequency) for ${province}`, sourceCount: totalMentions, timestamp }]
            ...
            velocity: {
                ...
                trace: [{ table: 'riskSignals', query: `today_vs_7d_avg for ${province}`, sourceCount: mentionsToday + mentionsLast7Days, timestamp }]
            } as GovernedMetric<number>,
            alignment: {
                ...METRIC_REGISTRY.GOVERNMENT_ALIGNMENT,
                value: finalAlignmentScore !== null ? Math.round(finalAlignmentScore) : null,
                unit: '%',
                confidence: budgets.length > 0 ? calcConfidence(totalMentions, 50) : 0,
                rating: getConfidenceRating(budgets.length > 0 ? calcConfidence(totalMentions, 50) : 0),
                trace: [{ table: 'planningBudgets', query: `province=${province}`, sourceCount: budgets.length, timestamp }],
                governanceNote: budgets.length === 0 ? 'A provincial budget dataset is required to calculate alignment.' : undefined
            } as GovernedMetric<number>
        };
        ...
        return {
            ...
            trends: signals.reduce((acc: any[], s) => {
            ...
            dataCoverage: {
                ...
                municipality: (signals.filter(s => s.municipality && s.municipality !== 'Unknown').length / (totalMentions || 1)) * 100,
...
export async function getRegionalMetadata(country: string = 'South Africa') {
    try {
        const q = query(collection(db, 'community_signals'), where('country', '==', country));
        const communities = await getDocs(q);
        const allData = communities.docs.map(d => d.data());
        
        const provinces = Array.from(new Set(allData.map(c => c.province).filter(p => !!p)));
        const municipalities = Array.from(new Set(allData.map(c => c.municipality).filter(m => !!m)));
...
export async function getProjectPortfolio(province?: string, municipality?: string) {
    try {
        const filters = [];
        ...
        const projectsSnap = await getDocs(query(collection(db, 'gic_projects'), ...filters, limit(50)));
...
export async function getMapIntelligence() {
    try {
        ...
        const signalsSnap = await getDocs(query(collection(db, 'riskSignals'), limit(100)));
        const strategicSnap = await getDocs(query(collection(db, 'gic_projects'), limit(100)));

        const communities = communitiesSnap.docs.map(d => ({ id: d.id, ...d.data() }));
        const signals = signalsSnap.docs.map(d => ({ id: d.id, ...d.data() }));
        const strategic = strategicSnap.docs.map(d => ({ id: d.id, ...d.data() }));
...
export async function getCommunityIntelligence(province?: string) {
    try {
        const filters = [];
        if (province && province !== "All Regions") filters.push(where('province', '==', province));
...
export async function getVulnerabilityIntelligence() {
    try {
        const signalsSnap = await getDocs(query(collection(db, 'riskSignals'), limit(100)));
        const signals = signalsSnap.docs.map(d => ({ id: d.id, ...(d.data() as any) }));
...
export async function getUniversalSignals(params: {
    ...
    try {
        const { country = 'South Africa', province = 'Gauteng', municipality, ward, domain } = params;
        ...
        const snap = await getDocs(query(collection(db, 'universal_signals'), ...filters, limit(20)));
 ...
 */
export async function performOnDemandSynthesis(context: string, signals: any[]) {
    try {
        const { geminiFlash, extractJsonObject } = await import("@/services/ai-service");
        
        const rawContent = signals.map(s => JSON.stringify(s.raw_payload)).join("\n\n");
        const prompt = `
...
export async function generateExecutiveBriefing(params: {
    ...
    try {
        ...
        const signalContext = params.signals.map(s => s.text || s.raw_payload?.text || "").join("\n").substring(0, 5000);
        const simContext = params.simulationData ? JSON.stringify(params.simulationData) : "No simulation data active.";
