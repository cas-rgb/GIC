...
export default function Sovereign3DMap({
    ...
    const data: MapDataPoint[] = useMemo(() => {
        const commData = communities.map(c => ({
            ...
            color: (c.riskScore > 70 ? [244, 63, 94] : [204, 163, 0]) as [number, number, number],
            metadata: c
        }));
            ...
            <div className="absolute top-8 left-8 z-20 pointer-events-none">
                <div className="p-6 bg-slate-900/90 backdrop-blur-xl border border-white/10 rounded-[2rem] shadow-2xl">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-white/50">3D Risk Telemetry</span>
                    </div>
                    <div className="space-y-1">
                        <h3 className="text-xl font-display font-black text-white">Regional Extrusion</h3>
                        <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest leading-relaxed">
                            Mapping {communities.length} Communities & {projects.length} GIC Projects
                        </p>
                    </div>
                </div>
            </div>
                ...
                {hoveredObject && (
                    ...
                    >
                        <div className="p-6 bg-white rounded-3xl shadow-gic-professional border border-slate-100 min-w-[280px]">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{hoveredObject.type}</p>
                            <h4 className="text-lg font-display font-black text-slate-900 mb-3">{hoveredObject.name}</h4>
                            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-2xl">
                                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                    {hoveredObject.type === 'community' ? 'Risk Intensity' : 'Budget Power'}
                                </span>
                                <span className={`text-sm font-black ${hoveredObject.type === 'community' ? 'text-rose-500' : 'text-gic-blue'}`}>
                    ...
                    >
                        <div className="flex items-center justify-between mb-10">
                            <div className="p-3 bg-white/5 rounded-2xl">
                                {selectedObject.type === 'community' ? <Users className="w-6 h-6 text-gic-gold" /> : <Activity className="w-6 h-6 text-gic-blue" />}
                            </div>
                            <button onClick={() => setSelectedObject(null)} className="p-3 hover:bg-white/10 rounded-full transition-colors">
                                <X className="w-6 h-6" />
                            </button>
                        </div>

                        <div className="mb-8">
                            <span className="text-[10px] font-black text-gic-gold uppercase tracking-[0.3em]">{selectedObject.type} Overview</span>
                            <h2 className="text-3xl font-display font-black mt-2 leading-tight">{selectedObject.name}</h2>
                        </div>

                        <div className="space-y-6">
                            <div className="p-6 bg-white/5 border border-white/10 rounded-[2rem]">
                                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-4">Strategic Pulse</p>
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center">
                                        <span className="text-xs font-bold text-slate-400">Regional Impact</span>
                                        <span className="text-sm font-black">{Math.round(selectedObject.value)}%</span>
                                    </div>
                            ...
                            <div className="p-6 bg-slate-950 rounded-[2rem] border border-white/5">
                                <div className="flex items-center gap-3 mb-4">
                                    ...
                                    <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Metadata Anchor</span>
                                </div>
                                <p className="text-sm font-medium text-slate-400 leading-relaxed italic">
                                    "Deep-space multispectral analysis indicates {selectedObject.value > 70 ? 'extreme pressure' : 'stable growth'} in this sector. Recommend immediate deployment of {selectedObject.type === 'community' ? 'social' : 'technical'} task force."
                                </p>
                            </div>
                        </div>
                    </motion.div>
