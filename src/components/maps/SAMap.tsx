...
export default function SAMap({
    ...
    useEffect(() => {
        // Story Mode Panning
        if (googleMapRef.current && storyStep !== undefined && communities.length > 0) {
    ...
    useEffect(() => {
        const initMap = async () => {
            ...
            const { HeatmapLayer } = await (loader as any).importLibrary('visualization');
            ...
            if (activeLayers.includes('signals')) {
                communities.forEach((node) => {
                    const matchesService = !selectedService || selectedService === 'apex' ||
                        (selectedService === 'water' && node.issue === 'Water') ||
                        (selectedService === 'roads' && node.issue === 'Roads') ||
                        (selectedService === 'health' && node.issue === 'Health') ||
                        (selectedService === 'planning' && node.issue === 'Town Planning') ||
                        (selectedService === 'structural' && node.issue === 'Structural');

                    const color = node.sentiment < 40 ? '#ef4444' : node.sentiment < 70 ? '#3b82f6' : '#22c55e';
            ...
            if (activeLayers.includes('demand') || activeLayers.includes('hotspot')) {
                ...
                const heatmapData = communities.map(node => {
                    const latLng = new (window as any).google.maps.LatLng(node.lat, node.lng);
                    // Hotspots focus on high-priority negative sentiment signals
                    const weight = isHotspot
                        ? (node.sentiment < 40 ? node.priority : 0)
                        : (node.priority / 10);
                ...
                {hoveredNode && (
                    ...
                    >
                        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-gic-professional min-w-[240px]">
                            <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-1">{hoveredNode.name}</p>
                            <div className="flex justify-between items-baseline mb-3">
                                <p className="text-xl font-display font-bold text-slate-900 tracking-tight">Level {hoveredNode.priority}</p>
                                <span className="text-[9px] font-black text-slate-500 uppercase tracking-widest">{hoveredNode.issue}</span>
                            </div>
                            <div className="h-[1px] w-full bg-slate-100 mb-3" />
                            <div className="flex items-center gap-4">
                                <div className="flex -space-x-3 overflow-hidden">
                                    {[1, 2].map(i => (
                                        <div key={i} className="w-8 h-8 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center">
                                            <Users className="w-3 h-3 text-slate-400" />
                                        </div>
                                    ))}
                                </div>
                                <span className="text-[8px] font-black text-slate-500 uppercase tracking-widest">{hoveredNode.influencers.length} Stakeholders Identified</span>
                            </div>
                        </div>
                    </motion.div>
                )}
