...
import { Filter, ChevronDown, DollarSign, Globe, ShieldAlert } from "lucide-react";
...
export default function StrategicFilterBar({ filters }: StrategicFilterBarProps) {
  return (
    <div className="flex flex-wrap items-center gap-4 p-4 bg-white/40 backdrop-blur-2xl border border-white/40 rounded-3xl shadow-xl">
      <div className="flex items-center gap-3 px-4 py-2 bg-slate-900/5 rounded-2xl border border-slate-900/5">
        ...
        <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Filter Strategy</span>
      </div>
      ...
      {filters.map((filter, i) => (
        <div key={i} className="relative group">
          <div className="absolute left-4 top-1/2 -translate-y-1/2 flex items-center gap-2 pointer-events-none">
            {React.isValidElement(filter.icon) && React.cloneElement(filter.icon as React.ReactElement<any>, { className: "w-3.5 h-3.5 text-slate-400" })}
          </div>
          <select
          ...
          >
            <option value="" disabled className="text-slate-400">{filter.label}</option>
