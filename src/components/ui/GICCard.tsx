...
export default function GICCard({ title, subtitle, icon, children, premium, gold, className = "", footer, visual, simulating }: GICCardProps) {
    const simulationRows = Array.from({ length: 12 }, (_, i) => {
        const hexSeed = ((i + 1) * 1543).toString(16).toUpperCase().padStart(4, "0").slice(-4);
        ...
        >
                    ...
                    <div className="p-10 font-mono text-[7px] text-gic-gold space-y-1 overflow-hidden">
                        {simulationRows.map((row) => (
                            <div key={row.id} className="flex gap-4">
                                ...
                                <span className="animate-pulse">INGESTING_GEOSPATIAL_{row.id}...</span>
            ...
            {(title || icon) && (
                <div className="relative z-10 mb-5 flex items-start justify-between border-b border-slate-100 pb-3">
                    <div className="flex items-center gap-3">
                        {icon && (
                            <div className={`rounded-xl border p-2.5 ${premium ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-slate-50 text-slate-500'}`}>
                                {icon}
                            </div>
                        )}
                        <div>
                            {title && <h3 className="mb-1 text-base font-display font-bold leading-tight tracking-tight text-slate-900 md:text-lg">{title}</h3>}
                            {subtitle && <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">{subtitle}</p>}
                        </div>
                    </div>
                </div>
            )}
            ...
            {visual && (
                <div className="absolute inset-0 z-0 overflow-hidden rounded-3xl opacity-20">
                    <img src={visual} alt="Card visual background" className="w-full h-full object-cover" />
