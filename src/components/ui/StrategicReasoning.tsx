...
import { BrainCircuit, Fingerprint, Database, Cpu, ArrowRight, ShieldCheck, Activity, Loader2 } from "lucide-react";
...
export default function StrategicReasoning() {
    const [reasoning, setReasoning] = useState<StrategicReasoningType | null>(null);
        ...
        return (
            <div className="flex flex-col items-center justify-center py-20 bg-slate-900/5 rounded-[2.5rem] border border-slate-100">
                ...
                <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em] animate-pulse">
                    Synthesizing Cross-Sector Intelligence...
                </p>
            </div>
        );
    }
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h4 className="text-sm font-bold text-slate-900 leading-none">AI Strategic Synthesis</h4>
                        <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1.5">Focus: {reasoning.primaryNode}</p>
                    </div>
                </div>
                <button 
                ...
                >
                    ...
                    <span className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Re-Verify Signals</span>
                </button>
            </div>

            <div className="grid grid-cols-3 gap-4 mb-8">
                {reasoning.groundingData.map((data, i) => (
                    <div key={i} className="p-4 bg-slate-50 border border-slate-100 rounded-[1.5rem] relative overflow-hidden group">
                        <div className="absolute top-0 right-0 p-2 opacity-5 group-hover:opacity-10 transition-opacity">
                            <Database className="w-10 h-10" />
                        </div>
                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] mb-2">{data.label}</p>
                        <p className="text-xl font-display font-bold text-slate-900">{data.value}</p>
                    </div>
                ))}
            </div>

            <div className="space-y-4">
                <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest pb-2 border-b border-slate-100">AI Reasoning Steps</p>
            ...
            <div className="mt-8 p-6 bg-slate-900 rounded-[2.5rem] text-white relative overflow-hidden">
                ...
                <div className="relative z-10">
                    <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-widest text-gic-blue">Synthesis Result</span>
                    </div>
                    <h4 className="text-lg font-bold mb-2">Confidence Alignment: {Math.round(reasoning.logicStrength * 100)}%</h4>
                    <p className="text-xs text-slate-400 leading-relaxed mb-6 max-w-sm">
                        This reasoning is grounded in {reasoning.groundingData.length} live signals across the National Corridor. Cross-sector impact verified via Gemini Institutional Engine.
                    </p>
