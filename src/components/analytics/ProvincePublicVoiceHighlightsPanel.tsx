...
interface ProvincePublicVoiceHighlightsPanelProps {
  province: string;
...
export default function ProvincePublicVoiceHighlightsPanel({
  province,
  issueFamily,
  days = 30,
}: ProvincePublicVoiceHighlightsPanelProps) {
    ...
    async function load() {
      ...
      try {
        ...
        setState({
          status: "error",
          message:
            error instanceof Error ? error.message : "Failed to load province public voice highlights",
        });
      }
    }

    void load();
  }, [province, issueFamily, days]);
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
        {rows.length === 0 ? (
          <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 text-sm font-medium text-slate-500 xl:col-span-3">
            No governed public-evidence highlights matched this province issue yet.
          </div>
        ) : (
          rows.map((row) => (
