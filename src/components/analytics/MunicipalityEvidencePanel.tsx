...
import { MunicipalityEvidenceResponse } from "@/lib/analytics/types";

interface MunicipalityEvidencePanelProps {
  province: string;
  municipality: string;
  topic?: string | null;
  ward?: string | null;
...
type LoadState =
  | { status: "loading" }
  | { status: "loaded"; data: MunicipalityEvidenceResponse }
  | { status: "error"; message: string };

export default function MunicipalityEvidencePanel({
  province,
  municipality,
  topic,
  ward = null,
}: MunicipalityEvidencePanelProps) {
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      ...
      try {
        ...
        const data = (await response.json()) as MunicipalityEvidenceResponse;
        ...
        setState({
          ...
          message:
            error instanceof Error
              ? error.message
              : "Failed to load municipality evidence",
        });
      }
    }

    void load();
  }, [province, municipality, topic, ward]);
