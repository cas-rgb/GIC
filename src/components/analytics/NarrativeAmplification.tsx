...
export default function NarrativeAmplification({ drivers }: { drivers: NarrativeDriver[] }) {
    if (!drivers || drivers.length === 0) {
        return (
            <div className="bg-slate-900 rounded-[3rem] p-10 h-full flex items-center justify-center border-4 border-slate-800">
                <span className="text-slate-500 font-display font-black uppercase tracking-widest animate-pulse">Analyzing Narrative Drivers...</span>
            </div>
        );
    }
        ...
        <div className="bg-slate-900 rounded-[3rem] p-10 h-full overflow-hidden border-4 border-slate-800 shadow-2xl relative">
            ...
            <div className="relative z-10 flex flex-col h-full">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h2 className="text-2xl font-display font-black text-white tracking-tighter uppercase mb-1 underline decoration-gic-blue decoration-4 underline-offset-8">Public Sentiment Analysis</h2>
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Cross-Channel Impact Analysis</p>
                    </div>
                    <div className="bg-gic-blue/10 px-4 py-2 rounded-full border border-gic-blue/20">
                        <span className="text-gic-blue text-[10px] font-black uppercase tracking-widest">Data Verified</span>
                    </div>
                </div>
                    ...
                    {drivers.map((driver, i) => (
                        ...
                        >
                            <div className="flex items-start justify-between mb-3">
                                <div>
                                    <h3 className="text-lg font-display font-black text-white mb-1">{driver.voice}</h3>
                                    <div className="flex items-center gap-3">
                                        <span className="text-[9px] font-black text-gic-blue uppercase tracking-widest">{driver.reach}</span>
                                        <div className="flex items-center gap-1 text-slate-500">
                                            <MessageSquare className="w-3 h-3" />
                                            <span className="text-[9px] font-black">{driver.citations} Citations</span>
                                        </div>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <div className="text-2xl font-display font-black text-white leading-none">{driver.impact}%</div>
                            ...
                            <div className="space-y-1">
                                <div className="flex items-center justify-between text-[8px] font-black uppercase tracking-[0.2em] text-slate-500 mb-1">
                                    ...
                                    <span className="text-gic-blue">{driver.leadershipScore}%</span>
                                </div>
