...
export default function CitizenVoiceTrendsPanel({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const response = await fetch(`/api/analytics/citizen-voice-trends?${query.toString()}`, {
          cache: "no-store",
        });
  ...
  const topicTrend = useMemo(() => {
    const dates = Array.from(new Set(data.issueTrend.map((row) => row.date))).sort((left, right) =>
    ...
    const series = data.issues.slice(0, 4).map((issue) => ({
      ...
      points: dates.map((date) => grouped.get(issue.issueFamily)?.get(date) ?? 0),
      mentionCount: issue.mentionCount,
      share: data.summary.mentionCount > 0 ? Math.round((issue.mentionCount / data.summary.mentionCount) * 100) : 0,
    }));
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-3 md:grid-cols-4">
        ...
        <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
          ...
          <p className="mt-1 text-xl font-display font-bold text-slate-900">
            {Math.round(data.summary.avgNegativeShare * 100)}%
          </p>
        </div>
        <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
      ...
      <div className="grid gap-5 xl:grid-cols-[1fr_1.02fr]">
              ...
              {topicTrend.series.map((series, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <p className="text-sm font-bold text-slate-900">{series.issueFamily}</p>
                      <p className="text-[10px] font-black uppercase tracking-[0.15em] text-slate-400">
                        {series.share}% share of voice
                      </p>
                    </div>
                  </div>
                  <p className="text-sm font-bold text-slate-900">{series.mentionCount}</p>
                </div>
              ))}
            </div>
          </div>
            ...
            {data.issues.slice(0, 4).map((row, index) => {
              const share =
                data.summary.mentionCount > 0
                  ? Math.round((row.mentionCount / data.summary.mentionCount) * 100)
                  : 0;
              const tone =
                index === 0
                ...
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="text-right">
                      <p className="text-2xl font-display font-bold text-slate-900">{share}%</p>
                  ...
                  <div className="mt-3 flex items-center justify-between text-[10px] font-black uppercase tracking-[0.18em]">
                    <span className="text-rose-600">{Math.round(row.negativeShare * 100)}% negative</span>
                    <span className="text-blue-600">{row.avgConfidence} confidence</span>
                  </div>
                </button>
              );
            })}
        ...
        <div className="rounded-2xl border border-slate-100 bg-white px-4 py-4">
          ...
          <div className="mt-4 overflow-hidden rounded-2xl border border-slate-100">
            <div className="grid grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr_0.8fr] gap-3 border-b border-slate-100 bg-slate-50 px-4 py-3 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
              <p>Issue</p>
            ...
            <div className="divide-y divide-slate-100">
              {data.issues.map((row) => (
                <button
                  ...
                  type="button"
                  onClick={() => onSelectIssueFamily?.(row.issueFamily)}
                  className={`grid w-full grid-cols-1 gap-3 px-4 py-3 text-left lg:grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr_0.8fr] lg:items-start ${
                    selectedIssueFamily === row.issueFamily
                      ? "bg-blue-50"
                      : "bg-white"
                  }`}
                >
                  ...
                  <div className="text-right">
                    <p className="text-sm font-bold text-blue-700">{row.avgConfidence}</p>
