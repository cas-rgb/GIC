...
export default function MunicipalRadar({ data }: { data: any }) {
    ...
    return (
        <div className="bg-white rounded-3xl border border-slate-100 p-5 h-full flex flex-col">
            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-4">Municipal Grounding Radar</span>
            <div className="flex-1 relative mt-2">
                <svg viewBox="0 0 100 100" className="w-full h-full overflow-visible">
                    {/* Radar Web */}
                    {[20, 40].map((r, i) => (
                        <circle key={i} cx="50" cy="50" r={r} fill="none" stroke="#f1f5f9" strokeWidth="0.5" />
                    ))}
                    {keys.map((_, i) => (
                        <line
                            key={i}
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-2 mt-4">
                {keys.map((key, i) => (
                    <div key={i} className="flex justify-between items-center bg-slate-50/50 p-2 rounded-xl border border-slate-100/50">
                        <span className="text-[7px] font-black text-slate-400 uppercase tracking-tighter truncate max-w-[60px]">{key.replace(/_/g, " ")}</span>
                        <span className="text-[8px] font-black text-slate-800">{Math.round(data[key])}%</span>
                    </div>
                ))}
            </div>
        </div>
