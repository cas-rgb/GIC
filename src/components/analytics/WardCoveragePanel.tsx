    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 gap-3 text-center md:grid-cols-6">
        <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
          <p className="text-lg font-bold text-slate-900">{data.summary.registryWardCount}</p>
          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">
            Wards Known
          </p>
        </div>
        <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
          <p className="text-lg font-bold text-slate-900">{data.summary.evidenceBackedWardCount}</p>
          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">
            Evidence-backed
          </p>
        </div>
        <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
          <p className="text-lg font-bold text-slate-900">{data.summary.documentCount}</p>
          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">
            Documents
          </p>
        </div>
        <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
          <p className="text-lg font-bold text-slate-900">{data.summary.pressureCaseCount}</p>
          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">
            Pressure
          </p>
        </div>
        <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
          <p className="text-lg font-bold text-slate-900">{data.summary.sentimentMentionCount}</p>
          <p className="text-[9px] font-black uppercase tracking-[0.2em] text-slate-400">
            Sentiment
          </p>
        </div>
        <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
          <span className={`inline-flex rounded-xl border px-3 py-1.5 text-[10px] font-black uppercase tracking-[0.2em] ${readinessTone}`}>
      ...
      {data.summary.wardReadinessLabel === "Registry Only" ? (
        <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3">
          <p className="text-sm font-medium text-slate-700">
            This municipality has known ward geography in the governed registry, but the current issue and evidence metrics are not yet ward-resolved. Use ward names for orientation only and rely on municipality-level KPIs plus evidence rows for decision-making.
          </p>
        </div>
      ) : null}
                ...
                data.rows.map((row) => (
                  <div
                    ...
                    className={`grid cursor-pointer grid-cols-1 gap-3 px-4 py-4 transition-colors lg:grid-cols-[1.2fr_0.8fr_0.8fr_0.8fr] lg:items-start ${
                      selectedWard === row.ward ? "bg-blue-50" : "bg-white hover:bg-slate-50"
                    }`}
                    onClick={() => onSelectWard?.(selectedWard === row.ward ? null : row.ward)}
                  >
                    ...
                    <p className="text-right text-sm font-bold text-slate-900">{row.pressureCaseCount}</p>
                    <p className="text-right text-sm font-bold text-slate-900">{row.sentimentMentionCount}</p>
                  </div>
                ))
              )}
            </div>
            ...
            <div className="max-h-[32rem] divide-y divide-slate-100 overflow-y-auto">
              {data.communityRows.length === 0 ? (
                ...
                data.communityRows.map((row) => (
                  ...
                  >
                    <p className="text-sm font-bold text-slate-900">{row.community}</p>
                    <p className="text-right text-sm font-bold text-slate-900">{row.documentCount}</p>
                    <p className="text-right text-sm font-bold text-amber-700">{row.avgUrgency.toFixed(1)}</p>
                    <p className="text-sm font-medium text-slate-700">{row.dominantIssue ?? "Mixed local complaints"}</p>
                  </div>
                ))
              )}
            </div>
      ...
      {data.summary.wardReadyCommunityCount > 0 ? (
        <div className="rounded-2xl border border-amber-100 bg-amber-50 px-4 py-3">
          <p className="text-sm font-medium text-slate-700">
            Formal ward mapping is still incomplete here, so the dashboard is also using {data.summary.wardReadyCommunityCount} ward-ready community rows from imported local evidence to preserve local visibility.
          </p>
        </div>
      ) : null}
