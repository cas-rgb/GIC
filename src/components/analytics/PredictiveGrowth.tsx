...
export default function PredictiveGrowth({ data }: { data: any[] }) {
  if (!data || data.length === 0)
    return (
      <div className="p-5 text-[8px] font-black text-white/30 uppercase italic">
        No Forecast Data Sync
      </div>
    );
  ...
  return (
    <div className="bg-slate-900 rounded-3xl border border-white/5 p-5 h-full flex flex-col text-white relative overflow-hidden">
      ...
      <span className="text-[9px] font-black text-white/40 uppercase tracking-widest mb-6 relative z-10">
      ...
      <div className="flex-1 flex items-end gap-3 px-2 mb-2 relative z-10">
        {data.map((d, i) => (
          ...
          >
            ...
            <span className="text-[10px] font-black text-gic-blue">
              +{Math.round(d.growth)}%
            </span>
          </div>
        ))}
