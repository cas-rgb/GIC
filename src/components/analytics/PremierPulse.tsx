...
export default function PremierPulse({ data, province }: { data: any, province: string }) {
    if (!data) return <div className="p-8 text-gray-400 font-black uppercase text-[10px]">No Executive Presence Data</div>;
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-8 h-full flex flex-col shadow-sm overflow-hidden relative group font-sans">
            ...
            <div className="relative z-10 flex items-center justify-between mb-8 pb-4 border-b border-gray-100">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        <h3 className="text-xl font-black text-gray-900 tracking-tight uppercase leading-none mb-1">{data.name}</h3>
                        <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest">{province} Executive Audit</p>
                    </div>
                </div>
                <div className="text-right">
                    <div className="text-3xl font-black text-gray-900 leading-none">{data.sentiment}%</div>
                    <span className="text-[8px] font-black text-blue-600 uppercase tracking-widest">Sentiment Audit</span>
                </div>
            </div>
            ...
            <div className="relative z-10 flex-1 flex flex-col gap-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {data.associates.map((associate: string, i: number) => (
                            ...
                            >
                                ...
                                <span className="text-[10px] font-black text-gray-600 uppercase tracking-tight">{associate}</span>
                            </motion.div>
                        ))}
                    </div>
                </div>
                ...
                <div className="mt-auto p-5 bg-gray-50 border border-gray-100 rounded-xl flex items-center justify-between group-hover:border-blue-400 transition-all">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <span className="text-[8px] font-black text-gray-400 uppercase tracking-widest">Policy Alignment</span>
                            <div className="text-xs font-black text-gray-900 uppercase">Strategic Mandate Alignment</div>
                        </div>
                    </div>
