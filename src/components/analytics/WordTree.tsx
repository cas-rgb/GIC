...
export default function WordTree({ data }: { data: any[] }) {
  if (!data || data.length === 0)
    return (
      <div className="p-8 text-slate-300 font-black uppercase text-[10px]">
        No Semantic Signals
      </div>
    );
  ...
  return (
    <div className="bg-slate-900 rounded-[2.5rem] p-10 h-full flex flex-col shadow-2xl relative overflow-hidden group">
        ...
        {sorted.map((item, i) => {
          ...
          return (
            <motion.div
              ...
              className="text-white font-display font-black tracking-tighter uppercase whitespace-nowrap cursor-pointer hover:text-blue-400 hover:opacity-100 transition-all px-2"
              style={{ fontSize: `${scale * 12}px` }}
              onClick={item.onClick}
            >
              {item.word}
              <span className="ml-1 text-[8px] opacity-30 font-sans tracking-normal bg-white/10 px-1 rounded">
                {item.count}
              </span>
            </motion.div>
          );
      ...
      <div className="mt-8 flex items-center gap-2 border-t border-white/5 pt-6 text-[8px] font-black text-white/40 uppercase tracking-widest">
        ...
        <span>Anchored in Common-Vector Logic</span>
      </div>
    </div>
  );
}
