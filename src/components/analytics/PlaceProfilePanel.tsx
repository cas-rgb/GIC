    ...
    async function load() {
      ...
      try {
        const response = await fetch(
          `/api/analytics/place-profile?${params.toString()}`,
          {
            cache: "no-store",
          },
        );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <Metric
          label="Population"
          value={formatNumber(demographics.populationTotal)}
        />
        <Metric
          label="Households"
          value={formatNumber(demographics.householdsTotal)}
        />
        <Metric
          label="Unemployment"
          value={formatPercent(demographics.unemploymentRate)}
        />
        <Metric
          label="Water Access"
          value={formatPercent(demographics.serviceAccessWater)}
        />
        <Metric
          label="Electricity"
          value={formatPercent(demographics.serviceAccessElectricity)}
        />
        <Metric
          label="Sanitation"
          value={formatPercent(demographics.serviceAccessSanitation)}
        />
      </div>
      <JsonGroup
        title="Language Profile"
        value={demographics.languageProfile}
      />
      <JsonGroup
        title="Settlement Profile"
        value={demographics.settlementProfile}
      />
      <JsonGroup
        title="Economic Profile"
        value={demographics.economicProfile}
      />
    </div>
  );
}
...
function Metric(props: { label: string; value: string }) {
  return (
    <div className="rounded-2xl border border-slate-800 bg-slate-900/50 px-4 py-3">
      ...
      <p className="mt-2 text-lg font-display font-bold text-blue-400">
        {props.value}
      </p>
    </div>
  );
    ...
    <div>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        {entries.map(([key, value]) => (
          <span
            key={key}
            className="rounded-full border border-slate-700 bg-slate-800 px-3 py-1 text-xs font-medium text-slate-300 shadow-sm"
          >
