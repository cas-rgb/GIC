...
type LoadState =
  | { status: "loading" }
  | { status: "loaded"; data: MunicipalityEvidenceBalanceResponse }
  | { status: "error"; message: string };

export default function MunicipalityEvidenceBalancePanel({
  province,
  municipality,
  days = 30,
}: MunicipalityEvidenceBalancePanelProps) {
  ...
  useEffect(() => {
    ...
    async function load(): Promise<void> {
      ...
      try {
        ...
        const data =
          (await response.json()) as MunicipalityEvidenceBalanceResponse;
        ...
        setState({
          ...
          message:
            error instanceof Error
              ? error.message
              : "Failed to load municipality evidence balance",
        });
      }
    }
    ...
    return (
      <div className="flex min-h-[260px] items-center justify-center">
        <div className="flex items-center gap-3 text-sm font-bold text-slate-400">
          ...
          Loading municipality evidence balance...
        </div>
      </div>
    );
  }
