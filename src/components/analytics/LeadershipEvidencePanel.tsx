    ...
    async function load() {
      ...
      try {
        const response = await fetch(
          `/api/analytics/leadership-evidence?${params.toString()}`,
          {
            cache: "no-store",
          },
        );
        ...
        setState({
          ...
          message:
            error instanceof Error
              ? error.message
              : "Failed to load leadership evidence",
        });
      }
    }
    ...
    <div className="space-y-4">
      <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
        ...
        <p className="mt-2 text-sm font-medium text-slate-700">
          This evidence pack shows the governed documents currently shaping the
          public-relations narrative for{" "}
          <span className="font-bold">{activeSubject}</span> in{" "}
      ...
      <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
        ...
        <div className="rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
          ...
          <p className="mt-1 text-sm font-bold text-slate-900">
            {state.data.summary.municipalityCount}
          </p>
        </div>
      </div>
          ...
          <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 text-sm font-medium text-slate-500 xl:col-span-3">
            {hasLeaderFilter ? (
              <>
                ...
                in <span className="font-bold text-slate-700">{province}</span>{" "}
                for the last{" "}
                <span className="font-bold text-slate-700">{days} days</span>.
                Clear the current leader filter to return to the wider province
                leadership evidence pack.
              </>
            ) : (
              <>
                No governed PR evidence highlights matched the current province
                and time window yet. This usually means the leadership evidence
                layer is still thin for this slice rather than that no political
                narrative exists.
              </>
            )}
          </div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-[10px] font-black uppercase tracking-[0.2em] text-slate-400">
                    {[province, row.municipality, row.sourceName]
                      .filter(Boolean)
                      .join(" | ")}
                  </p>
                </div>
