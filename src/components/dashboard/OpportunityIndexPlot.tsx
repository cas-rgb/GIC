...
export default function OpportunityIndexPlot({
  ...
  const gautengData = SA_REGIONAL_REGISTRY.provinces.find(
    (p) => p.name === "Gauteng",
  );
  ...
  const chartData = React.useMemo(() => {
    ...
    const municipalitiesTyped: Record<
      string,
    ...
    const filteredPlanning = planning.filter(
      (p) =>
        ...
        (activeMunicipality === "all" ||
          p.municipality === activeMunicipality) &&
    ...
    const filteredRisks = risks.filter(
      (r) =>
        ...
        (activeMunicipality === "all" ||
          r.municipality === activeMunicipality) &&
    ...
    const filteredTenders = tenders.filter(
      (t) =>
        ...
        (activeMunicipality === "all" ||
          t.municipality === activeMunicipality) &&
    ...
    filteredPlanning.forEach((p) => {
      const mun =
        activeWard !== "all"
          ...
          : activeMunicipality !== "all"
            ? p.municipality
            : p.municipality || p.province || "National";
      if (!municipalitiesTyped[mun])
        municipalitiesTyped[mun] = { x: 0, y: 0, z: 0, name: mun };
    ...
    filteredRisks.forEach((r) => {
      const mun =
        activeWard !== "all"
          ...
          : activeMunicipality !== "all"
            ? r.municipality
            : r.municipality || r.province || "National";
      if (!municipalitiesTyped[mun])
        municipalitiesTyped[mun] = { x: 0, y: 0, z: 0, name: mun };
    ...
    filteredTenders.forEach((t) => {
      const mun =
        activeWard !== "all"
          ...
          : activeMunicipality !== "all"
            ? t.municipality
            : t.municipality || t.province || "National";
      if (!municipalitiesTyped[mun])
        municipalitiesTyped[mun] = { x: 0, y: 0, z: 0, name: mun };
    ...
    <div className="w-full h-[400px] bg-slate-900 border border-slate-800 rounded-[3rem] p-10 flex flex-col relative overflow-hidden group">
      ...
      <div className="flex items-center justify-between mb-8 relative z-10">
        <div>
          ...
          <p className="text-[10px] text-slate-400 uppercase tracking-[0.3em] font-black mt-1">
            Complexity (Risk) vs. Budget (Allocation)
          </p>
        </div>
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                  activeSector === s.value
                    ? "bg-gic-blue text-white shadow-lg"
        ...
        {chartData.length > 0 ? (
          ...
          <div className="text-center">
            ...
            <p className="text-[10px] text-slate-500 uppercase tracking-widest font-black">
              Refine filters or check GIC database connection
            </p>
          </div>
        )}
      </div>
      ...
      <div className="mt-4 flex items-center gap-3 p-3 bg-white/5 border border-white/5 rounded-2xl relative z-10">
        ...
        <p className="text-[10px] font-bold text-slate-300 italic">
          Top-Right Quadrant clusters represent{" "}
          <span className="text-white font-black">Strategic Priorities</span>{" "}
          for GIC Boardroom interventions.
        </p>
      </div>
    </div>
  );
