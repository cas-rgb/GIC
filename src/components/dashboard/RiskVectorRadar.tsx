...
export default function RiskVectorRadar({ data }: RiskVectorRadarProps) {
  const vectors = React.useMemo(() => {
    ...
    return categories.map((cat) => {
      const signals = data.filter(
        (s) => s.domain === cat.sector || s.category === cat.sector,
      );
      const avgSeverity =
        signals.length > 0
          ? (signals.reduce(
              (acc, s) => acc + (s.severityValue || s.severity || 5),
              0,
            ) /
              signals.length) *
            10
  ...
  const aggregateRisk = Math.round(
    vectors.reduce((acc, v) => acc + v.value, 0) / (vectors.length || 1),
  );
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-xs text-white/40">
            Multi-dimensional vulnerability audit
          </p>
        </div>
      </div>
            ...
            <div key={v.label} className="group cursor-default">
              <div className="flex justify-between items-center mb-1.5">
                <div className="flex items-center gap-2">
                  <div
                    className={`p-1.5 rounded-lg bg-white/5 text-white/40 group-hover:text-white transition-colors`}
                  >
                    {React.isValidElement(v.icon) &&
                      React.cloneElement(
                        v.icon as React.ReactElement<{ className?: string }>,
                        { className: "w-3 h-3" },
                      )}
                  </div>
                  <span className="text-[10px] font-bold text-white/60 group-hover:text-white uppercase tracking-wider">
                    {v.label}
                  </span>
                </div>
