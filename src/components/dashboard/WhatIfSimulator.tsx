...
export default function WhatIfSimulator() {
  ...
  const prediction = useMemo(() => {
    ...
    sdi =
      40 + waterVal * 0.4 + healthVal * 0.3 + roadsVal * 0.2 + safetyVal * 0.1;
    sentiment =
      30 + safetyVal * 0.5 + healthVal * 0.3 + waterVal * 0.1 + roadsVal * 0.1;
  ...
  const handleBudgetChange = (id: string, newVal: number) => {
    ...
    setBudgets((prev) =>
      prev.map((b) => (b.id === id ? { ...b, value: newVal } : b)),
    );
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-12 gap-6">
          <div className="space-y-2">
            <div className="flex items-center gap-3">
              ...
              <span className="text-[10px] font-black text-gic-blue uppercase tracking-[0.5em]">
                Generation 4 Vision
              </span>
            </div>
            ...
            <p className="text-slate-400 font-medium max-w-xl text-lg">
              Manipulate resource allocation vectors to predict the trajectory
              of provincial stability and public sentiment.
            </p>
          </div>
          ...
          <div className="flex items-center gap-4 bg-white/5 border border-white/10 p-4 rounded-3xl backdrop-blur-xl">
            <div className="space-y-0.5">
              ...
              <div className="flex items-baseline gap-2">
                <span
                  className={`text-2xl font-black ${isOverBudget ? "text-rose-500" : "text-emerald-500"}`}
                >
                  {totalPoints}
                </span>
            ...
            {budgets.map((sector) => (
              <div key={sector.id} className="group">
                <div className="flex justify-between items-center mb-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-2 bg-white/5 rounded-xl border border-white/5 group-hover:border-white/20 transition-all`}
                    >
                  ...
                  <span className={`text-lg font-black ${sector.color}`}>
                    {sector.value}%
                  </span>
                </div>
                <div className="relative h-2 bg-white/5 rounded-full overflow-hidden group-hover:bg-white/10 transition-all">
                  ...
                  <input
                    ...
                    value={sector.value}
                    onChange={(e) =>
                      handleBudgetChange(sector.id, parseInt(e.target.value))
                    }
            ...
            <div className="pt-6 border-t border-white/10">
              <div className="flex items-center gap-4 text-slate-500">
                ...
                <p className="text-xs font-bold uppercase tracking-widest">
                  Modeling: Weighted Impact Probability Engine V4.2
                </p>
              </div>
            </div>
          </div>
              ...
              {isSimulating && (
                ...
                >
                  <div className="flex flex-col items-center gap-4">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-[0.4em] text-gic-blue">
            ...
            <div className="space-y-12 h-full flex flex-col justify-center">
              ...
              <div className="grid grid-cols-2 gap-8">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-3">
                    <div
                      className={`p-1 rounded-lg ${prediction.delta >= 0 ? "bg-emerald-500/20" : "bg-rose-500/20"}`}
                    >
                      <TrendingUp
                        className={`w-5 h-5 ${prediction.delta >= 0 ? "text-emerald-500" : "text-rose-500 transform rotate-180"}`}
                      />
                    </div>
                    <span
                      className={`text-2xl font-black ${prediction.delta >= 0 ? "text-emerald-500" : "text-rose-500"}`}
                    >
                      {prediction.delta > 0 ? "+" : ""}
                      {prediction.delta}%
                    </span>
