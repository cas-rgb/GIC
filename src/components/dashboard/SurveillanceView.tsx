                    ...
                    <AnimatePresence mode="wait">
                        {selectedNode ? (
                            ...
                            >
                                ...
                                <div className="p-6 bg-slate-900 border border-white/10 rounded-[2.5rem] relative overflow-hidden group">
                                    ...
                                    <h4 className="text-2xl font-black text-white leading-tight mb-4">
                                        {selectedNode.projectName || selectedNode.name || "Strategic Data Node"}
                                    </h4>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                                        <p className="text-[9px] font-black text-slate-400 uppercase mb-1">Risk Index</p>
                                        <p className="text-xl font-black text-slate-900">{selectedNode.riskLevel || '74.2'}</p>
                                    </div>
                                    <div className="p-4 bg-slate-50 border border-slate-100 rounded-2xl">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="bg-slate-50 border border-slate-100 rounded-[2rem] p-6 space-y-6">
                                        {Object.entries(selectedNode)
                                            .filter(([k]) => !['id', 'lat', 'lng', 'projectName', 'name'].includes(k))
                                            .map(([key, value]) => (
                                                <div key={key} className="flex justify-between items-start border-b border-slate-100 pb-3 last:border-0 last:pb-0">
                                                    <span className="text-[10px] font-black text-slate-400 uppercase">{key.replace(/([A-Z])/g, ' $1').trim()}</span>
                            ...
                            <div className="flex flex-col items-center justify-center py-20 text-center">
                                ...
                                <h4 className="text-slate-900 font-black mb-2">Awaiting Node Selection</h4>
                                <p className="text-xs text-slate-400 max-w-[200px] font-medium leading-relaxed">
                                    Select an active surveillance point on the map to extract high-fidelity intelligence.
                                </p>
                            </div>
                        )}
                    </AnimatePresence>
                ...
                <div className="p-8 bg-slate-950 rounded-[2.5rem] border border-white/5 text-white flex items-center justify-between shadow-lg">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-lg font-black tracking-tight">Real-Time Context</p>
                        </div>
                    </div>
                    <div className="flex items-center gap-2 px-3 py-1 bg-blue-500/10 rounded-full border border-blue-500/20">
                         <div className="w-2 h-2 rounded-full bg-blue-500 animate-pulse" />
                         <span className="text-[8px] font-black text-blue-400 uppercase">Polling</span>
                    </div>
                </div>
            </div>
        </div>
