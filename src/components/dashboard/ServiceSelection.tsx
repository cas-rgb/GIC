...
export const serviceAreas: ServiceArea[] = [
    {
        ...
        color: 'blue',
        category: 'Civil',
        metrics: { risk: dbServiceMetrics['water']?.risk || 42, sentiment: 45, volume: 1240 }
    },
    {
        id: 'roads',
        title: 'Roads & Works',
        ...
        color: 'emerald',
        category: 'Roads',
        metrics: { risk: dbServiceMetrics['roads']?.risk || 68, sentiment: 32, volume: 842 }
    },
    {
        id: 'health',
        ...
        color: 'rose',
        category: 'Health',
        metrics: { risk: dbServiceMetrics['health']?.risk || 15, sentiment: 78, volume: 210 }
    },
    {
        id: 'planning',
        title: 'Town Planning',
    ...
    return (
        <div className="min-h-screen bg-white overflow-y-auto scrollbar-hide pb-20">
            ...
            <div className="max-w-[1600px] mx-auto px-20 -mt-12 relative z-20 grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                    { label: 'Highest Regional Risk', value: '82%', target: 'Gauteng Water', icon: AlertCircle, color: 'text-red-500' },
                    { label: 'Narrative Velocity', value: '+14%', target: 'Roads / Works', icon: TrendingUp, color: 'text-blue-500' },
                    { label: 'Audit Coverage', value: '98.4%', target: 'Active Mandate', icon: Zap, color: 'text-gray-400' }
                ].map((item, i) => (
                    <motion.div
                        key={i}
                    ...
                    >
                        <div className="flex justify-between items-start mb-6">
                            ...
                            <span className="text-[10px] font-black uppercase tracking-[0.3em] text-gray-400">{item.label}</span>
                        </div>
                        <div className="text-4xl font-black text-gray-900 mb-1">{item.value}</div>
                        <div className="text-sm font-bold text-gray-500 tracking-tight">{item.target}</div>
                    </motion.div>
                ))}
            </div>
            ...
            <div className="max-w-[1600px] mx-auto p-20 space-y-16">
                <div>
                    <h2 className="text-3xl font-black text-gray-900 uppercase tracking-tight">Sectoral Report Hub</h2>
                    <p className="text-lg font-bold text-gray-400 leading-tight max-w-2xl pt-2">Select a specialized infrastructure sector to access deep-dive compliance audits and real-time intervention framing.</p>
                </div>
                        ...
                        >
                            <div className="flex justify-between items-start mb-8">
                                ...
                                <div className="text-right">
                                    <div className="text-[8px] font-black uppercase tracking-widest text-gray-300">Report ID</div>
                                    <div className="text-xs font-bold text-gray-500">{service.id.toUpperCase()}</div>
                                </div>
                            </div>
                            ...
                            <div className="mt-8 pt-8 border-t border-gray-100 grid grid-cols-2 gap-6">
                                <div>
                                    <div className="text-[8px] font-black uppercase tracking-widest text-gray-400 mb-2">Operational Risk</div>
                                    <div className={`text-2xl font-black ${service.metrics.risk > 60 ? 'text-red-600' : 'text-gray-900'}`}>
                                        {service.metrics.risk}%
                                    </div>
                                </div>
                                <div>
                                    <div className="text-[8px] font-black uppercase tracking-widest text-gray-400 mb-2">Audit Volume</div>
            ...
            <div className="mx-20 p-20 bg-slate-900 rounded-[4rem] text-center border-[8px] border-slate-100">
                <div className="flex justify-center gap-12 text-white/40 mb-10">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[10px] font-black uppercase tracking-[0.4em]">Search: Grounded</span>
                    </div>
                    <div className="flex items-center gap-3">
                        <div className="w-3 h-3 rounded-full bg-gic-blue" />
                        <span className="text-[10px] font-black uppercase tracking-[0.4em]">Vertex AI: Optimized</span>
                    </div>
                </div>
                <h2 className="text-display-md text-white mb-6">GIC | Community Intelligence Platform</h2>
                <p className="text-xl text-white/50 font-medium">Strategic Infrastructure Governance for the Gauteng Infrastructure Company.</p>
            </div>
        </div>
    );
}
