    ...
    return (
        <>
            {signals.slice(0, 5).map((signal, i) => {
                ...
                return (
                    <Float key={signal.id || i} speed={2} rotationIntensity={0.5} floatIntensity={1} position={[x, y, z]}>
                        <mesh>
                            <sphereGeometry args={[0.2, 16, 16]} />
                            <meshStandardMaterial 
                                color={signal.sentiment === 'negative' ? '#ef4444' : '#3b82f6'} 
                                emissive={signal.sentiment === 'negative' ? '#ef4444' : '#3b82f6'}
                                emissiveIntensity={2}
                            />
                        </mesh>
                        <Text
...
export default function IntelligencePulse({ signals = [] }: { signals: any[] }) {
    return (
        <div className="w-full h-[400px] bg-slate-950 rounded-[2.5rem] overflow-hidden border border-white/5 relative group">
            ...
            <div className="absolute bottom-6 left-6 p-4 bg-slate-900/50 backdrop-blur-md rounded-2xl border border-white/10 pointer-events-none">
                <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-[9px] font-black text-white uppercase tracking-widest">Neural Signal Feed</span>
                </div>
            ...
            <div className="absolute top-6 right-6 flex items-center gap-2">
                <div className="px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                    <span className="text-[8px] font-black text-emerald-500 uppercase tracking-widest">Fidelity: HIGH</span>
                </div>
            </div>
        </div>
    );
