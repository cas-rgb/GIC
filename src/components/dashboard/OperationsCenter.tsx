...
import { MessageSquare, ExternalLink, ShieldCheck, MapPin, Zap, Database, Search, ShieldAlert, Eye, Radio, Activity } from 'lucide-react';
...
export default function OperationsCenter({ serviceId, serviceCategory }: OperationsCenterProps) {
    ...
    useEffect(() => {
        ...
        const q = serviceId === 'apex'
            ? query(collection(db, "community_signals"), orderBy("timestamp", "desc"), limit(100))
            : query(collection(db, "community_signals"), where("category", "==", serviceCategory), orderBy("timestamp", "desc"), limit(100));

        const unsubscribe = onSnapshot(q, (snapshot) => {
            const docs = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as CommunitySignal));
            setState({
                signals: docs,
                isLoading: false,
                queryKey: currentQueryKey,
    ...
    return (
        <div className="p-8 h-full flex flex-col space-y-6 overflow-hidden">
            ...
            <div className="flex justify-between items-end">
                <div>
                    <h3 className="text-2xl font-display font-bold text-slate-900 tracking-tight">Infrastructure Signal Feed</h3>
                    ...
                    {filteredSignals.map((signal, idx) => (
                        ...
                        >
                            ...
                            <div className="flex flex-col items-center gap-2 w-12 border-r border-slate-50 pr-5">
                                <div className={`w-3 h-3 rounded-full ${signal.sentiment === 'negative' ? 'bg-rose-500 shadow-[0_0_10px_rgba(244,63,94,0.4)]' : 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.4)]'}`} />
                                <span className="text-[8px] font-black text-slate-400 uppercase tracking-tighter">Urgency</span>
                                <span className={`text-[10px] font-display font-black leading-none ${signal.sentiment === 'negative' ? 'text-rose-600' : 'text-emerald-600'}`}>
                                    {signal.urgency}
                                </span>
                            </div>
                            ...
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-3 mb-1">
                                    <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest">{signal.community || "Regional Node"}</span>
                                    <span className="text-[9px] font-bold text-slate-400 bg-slate-50 px-2 py-0.5 rounded-lg border border-slate-100">{signal.platform || "Direct feed"}</span>
                                </div>
                                <p className="text-[11px] text-slate-600 font-medium leading-relaxed truncate max-w-full">
                                    "{signal.evidence || signal.issue || "Infrastructure stress signature detected."}"
                                </p>
                            </div>
                            ...
                            <div className="flex items-center gap-6 pl-5 border-l border-slate-50">
                                <div className="flex flex-col items-end gap-1">
                                    <div className="flex items-center gap-1.5 opacity-40">
                                        ...
                                        <span className="text-[8px] font-black uppercase tracking-widest">Entry ID: {signal.id.slice(0, 5)}</span>
                                    </div>
                                    <span className="text-[8px] font-black text-slate-300 uppercase tracking-widest">
                                        {signal.timestamp?.seconds ? new Date(signal.timestamp.seconds * 1000).toLocaleTimeString() : 'LIVE'}
                                    </span>
                                </div>
                                {signal.source && signal.source !== "#" && (
                                    <a href={signal.source} target="_blank" className="p-2.5 bg-slate-50 text-slate-400 hover:text-gic-blue hover:bg-white border border-transparent hover:border-gic-blue rounded-xl transition-all shadow-sm">
                ...
                <div className="col-span-12 lg:col-span-7 bg-slate-900 rounded-[2rem] border border-white/5 overflow-hidden relative group h-[280px]">
                    ...
                    <div className="absolute bottom-6 left-8 right-8 flex justify-between items-end">
                        <div>
                            <div className="flex items-center gap-2 mb-2">
                                ...
                                <span className="text-[10px] font-black text-white/40 uppercase tracking-[0.2em]">Tactical Site Scan</span>
                            </div>
                            <h4 className="text-xl font-display font-black text-white tracking-tighter">Alexandra Sector 4</h4>
                            <p className="text-[10px] text-white/30 font-medium">Real-time Satellite Integrity Monitoring Active</p>
                        </div>
                    ...
                    <div className="absolute top-6 right-8">
                        <div className="flex items-center gap-3 px-3 py-1.5 bg-slate-800/40 border border-white/10 rounded-full">
                            <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Zone Monitored</span>
                        </div>
                    </div>
                </div>
                ...
                <div className="col-span-12 lg:col-span-5 bg-white/5 border border-white/10 rounded-[2rem] p-6 flex flex-col justify-between">
                    <div className="space-y-4">
                        <div className="flex items-center gap-3">
                            ...
                            <h3 className="text-lg font-bold text-slate-900 font-display">Active Tactical Directives</h3>
                        </div>
            ...
            <div className="h-14 mt-4 bg-slate-900 rounded-[1.8rem] border border-white/5 flex items-center justify-between px-8">
                <div className="flex gap-8">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-[9px] font-black text-white/50 uppercase tracking-[0.2em]">Data Feed: Grounded 2024</span>
                    </div>
                    <div className="flex items-center gap-3 border-l border-white/10 pl-8">
                        <ShieldCheck className="w-3.5 h-3.5 text-blue-400" />
                        <span className="text-[9px] font-black text-white/50 uppercase tracking-[0.2em]">Verification: Strategic Grounding Applied</span>
                    </div>
                </div>
                <div className="flex items-center gap-2">
                    <Zap className="w-3.5 h-3.5 text-amber-500" />
                    <span className="text-[9px] font-black text-white uppercase tracking-widest">Global Analytics: Processing {filteredSignals.length} Active Nodes</span>
                </div>
            </div>
        </div>
    );
...
function TacticalActionItem({ icon, title, description, status }: any) {
    return (
        <div className="flex items-center gap-4 p-3 bg-white border border-slate-100 rounded-xl hover:border-gic-blue transition-all cursor-pointer group">
            ...
            <div className="flex-1 min-w-0">
                <h5 className="text-[11px] font-bold text-slate-900 truncate">{title}</h5>
                <p className="text-[9px] text-slate-400 font-medium truncate">{description}</p>
            </div>
            <span className={`text-[8px] font-black px-2 py-0.5 rounded-full ${
                status === 'ACTIVE' ? 'bg-amber-100 text-amber-600' :
                status === 'STANDBY' ? 'bg-rose-100 text-rose-600' : 'bg-slate-100 text-slate-400'
            }`}>
                {status}
            </span>
        </div>
