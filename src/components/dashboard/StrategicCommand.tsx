...
export default function StrategicCommand({ serviceId, strategicInsights, isLoading }: StrategicCommandProps) {
    ...
    const SERVICE_IDENTITY: any = {
        'water': { color: 'text-blue-500', bg: 'bg-blue-600', icon: Droplets, mission: 'Supply Integrity & Wastewater Audit' },
        'roads': { color: 'text-emerald-500', bg: 'bg-emerald-600', icon: Truck, mission: 'Network Flow & Logistical Resilience' },
        'health': { color: 'text-rose-500', bg: 'bg-rose-600', icon: HeartPulse, mission: 'Facility Capacity & Clinical Reach' },
        'planning': { color: 'text-amber-500', bg: 'bg-amber-600', icon: Building2, mission: 'Urban Growth & Settlement Logic' },
        'structural': { color: 'text-indigo-500', bg: 'bg-indigo-600', icon: Construction, mission: 'Housing Safety & Structural Audit' },
        'apex': { color: 'text-gic-blue', bg: 'bg-slate-900', icon: ShieldCheck, mission: 'Unified Infrastructure Governance' }
    };
    ...
    const MUNICIPALITIES: any = {
        ...
        "Western Cape": ["All", "City of Cape Town", "Stellenbosch", "George", "Drakenstein"],
    ...
    const getServiceLead = (associates: string[]) => {
        ...
        return associates.find((a: string) => {
            ...
            if (serviceId === 'water') return lower.includes('water') || lower.includes('infrastructure') || lower.includes('cogta');
            if (serviceId === 'roads') return lower.includes('transport') || lower.includes('roads') || lower.includes('mobility') || lower.includes('infrastructure');
            if (serviceId === 'health') return lower.includes('health');
            if (serviceId === 'planning' || serviceId === 'structural') return lower.includes('human settlements') || lower.includes('infrastructure') || lower.includes('housing');
            return lower.includes('infrastructure') || lower.includes('public works');
        });
    };

    const generateHeatmapData = (prov: string) => {
        return MUNICIPALITIES[prov].filter((m: string) => m !== 'All').map((m: string) => ({
            name: m,
    ...
    const generatePromises = (prov: string) => {
        return [
            { id: '1', statement: `Upgrade ${prov} water treatment plants by Q4 2026`, date: 'Mar 2025', status: 'in_progress', sentiment: 45 },
            { id: '2', statement: `Eradicate illegal mining (Zama Zamas) in high-risk zones`, date: 'Feb 2025', status: 'risk', sentiment: 22 },
            { id: '3', statement: `Resurface 500km of provincial arterial roads`, date: 'Jan 2025', status: 'fulfilled', sentiment: 78 }
        ] as any[];
    };
    ...
    useEffect(() => {
        const syncLogic = async () => {
            ...
            const communityId = (municipality !== 'All' ? municipality : province).toLowerCase().replace(/\s+/g, "_");
            ...
            const [resLogic, resNews, leadershipDoc, promiseDoc] = await Promise.all([
                ...
                getStrategicNews(`${municipality !== 'All' ? municipality : province} ${serviceId === 'apex' ? 'infrastructure' : serviceId}`),
                getDoc(doc(db, "leadership", PREMIERS[province].toLowerCase().replace(/\s+/g, "_"))),
                getDoc(doc(db, "provincial_promises", provinceId))
            ]);

            // 2. Fetch Grounded Evidence (Seeded high-fidelity signals)
            const resGrounded = await getGroundedSignals(communityId, identity.category || 'Apex');

            // 3. Update States
            if (leadershipDoc.exists()) setLeadershipData(leadershipDoc.data());
            if (promiseDoc.exists()) setRealPromises(promiseDoc.data().promises || []);
            ...
            const groundedSignals: any[] = resGrounded.success ? (resGrounded.data || []) : [];

            const combinedContent = [
                ...groundedSignals.map((s: any) => `${s.title || ''}. ${s.excerpt || ''}`),
                ...baseArticles.map((a: any) => `${a.title || ''}. ${a.description || ''}`)
            ].join(" ");
            ...
            if (combinedContent.trim()) {
                ...
                if (resRisk.success) {
                    ...
                    const resRec = await getStrategicRecommendationsAction(resRisk.data || [], province);
    ...
    const insights = strategicInsights || { hotspots: [], lookalikes: [], interventionBrief: null };
        ...
        <div className="bg-[#F8F9FA] min-h-screen overflow-y-auto scrollbar-hide pb-20 font-sans">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 max-w-[1600px] mx-auto">
                    <div className="flex items-center gap-8">
                        ...
                        <div className="space-y-1">
                            ...
                            <h1 className="text-3xl font-black text-gray-900 tracking-tight uppercase">
                                {serviceId === 'apex' ? 'Integrated Strategic Overview' : identity.title}
                            </h1>
                            <p className="text-sm font-medium text-gray-500">{identity.mission}</p>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 bg-gray-50 p-2 rounded-xl border border-gray-200">
                        <div className="flex items-center gap-3 px-4 border-r border-gray-200">
                            ...
                            <select
                                value={province}
                                onChange={(e) => { setProvince(e.target.value); setMunicipality("All"); }}
                        ...
                        <div className="flex items-center gap-3 px-4">
                            ...
                            <select
                                ...
                                className="bg-transparent font-bold text-gray-700 text-sm focus:outline-none appearance-none pr-6"
                            >
                                {MUNICIPALITIES[province].map((m: string) => <option key={m} value={m}>{m}</option>)}
                            </select>
                        </div>
                        <div className="flex items-center gap-3 px-6 py-2 bg-white rounded-lg shadow-sm border border-gray-200">
                            <div className="w-2 h-2 rounded-full bg-blue-500" />
                            <span className="text-[10px] font-black text-gray-500 uppercase">Grounded Intelligence</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-[1600px] mx-auto px-12 pt-12 space-y-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                    <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Risk Exposure Index</span>
                            <AlertCircle className="w-5 h-5 text-red-500" />
                        </div>
                        <div className="text-5xl font-black text-gray-900">82.4%</div>
                        <div className="mt-4 flex items-center gap-2">
                            <TrendingUp className="w-4 h-4 text-red-500" />
                            <span className="text-xs font-bold text-red-500">+4.2% from last period</span>
                        </div>
                    </div>

                    <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Community Sentiment</span>
                            <MessageSquare className="w-5 h-5 text-gray-400" />
                        </div>
                        <div className="text-5xl font-black text-gray-900">45%</div>
                        <div className="mt-4 flex items-center gap-2">
                            <TrendingUp className="w-4 h-4 text-green-500" />
                            <span className="text-xs font-bold text-green-500">+12% positive shift</span>
                        </div>
                    </div>

                    <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
                        <div className="flex items-center justify-between mb-4">
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Signal Volume</span>
                            <Network className="w-5 h-5 text-gray-400" />
                        </div>
                        <div className="text-5xl font-black text-gray-900">3,283</div>
                        <div className="mt-4 text-xs font-bold text-gray-400 uppercase tracking-widest text-center">Data points audited</div>
                    </div>

                    <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm border-l-8 border-l-gic-blue">
                        <div className="flex items-center justify-between mb-4">
                            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Ministerial Lead</span>
                        ...
                        <div className="mt-4 text-[10px] font-black text-gray-400 uppercase tracking-widest">Portfolio Oversight</div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 xl:grid-cols-3 gap-12">
                    <div className="xl:col-span-2 space-y-6">
                        <div className="flex items-center justify-between border-b border-gray-200 pb-4">
                            <h2 className="text-xl font-black text-gray-900 tracking-tight uppercase">Regional Vulnerability Analysis</h2>
                            <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Spatial Risk Distribution</div>
                        </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center justify-between border-b border-gray-200 pb-4">
                            <h2 className="text-xl font-black text-gray-900 tracking-tight uppercase">Decision Lead Status</h2>
                            <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Leadership Node</div>
                        </div>
                        <div className="space-y-8">
                            ...
                            />
                            <div className="bg-white p-8 rounded-2xl border border-gray-200 shadow-sm">
                                <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest block mb-6">Sectoral Summary</span>
                                <div className="space-y-6">
                                    <p className="text-lg font-bold text-gray-700 leading-relaxed italic border-l-4 border-gray-200 pl-6">
                                        {recommendation?.mitigationStrategy || "Auditing regional data streams..."}
                                    </p>
                                    <div className="pt-6 border-t border-gray-100 flex items-center justify-between">
                                        <div className="text-xs font-black text-gray-400 uppercase tracking-widest">System Status</div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
                    <div className="space-y-6">
                        <div className="flex items-center justify-between border-b border-gray-200 pb-4">
                            <h2 className="text-xl font-black text-gray-900 tracking-tight uppercase">Sentiment Analysis Feed</h2>
                            <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Public Narrative Assessment</div>
                        </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center justify-between border-b border-gray-200 pb-4">
                            <h2 className="text-xl font-black text-gray-900 tracking-tight uppercase">Risk Forecasting Matrix</h2>
                            <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Predictive Modeling</div>
                        </div>
                        <div className="h-[700px] bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
                ...
                <div className="grid grid-cols-1 xl:grid-cols-4 gap-12">
                    <div className="xl:col-span-2 space-y-6">
                        <div className="flex items-center justify-between border-b border-gray-200 pb-4">
                            <h2 className="text-xl font-black text-gray-900 tracking-tight uppercase">Provincial Commitment Audit</h2>
                            <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Strategic Compliance Monitor</div>
                        </div>
                    ...
                    <div className="xl:col-span-2 space-y-6">
                        {recommendation && (
                            <>
                                <div className="flex items-center justify-between border-b border-gray-200 pb-4">
                                    <h2 className="text-xl font-black text-gray-900 tracking-tight uppercase">Executive Action Plan</h2>
                                    <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Intervention Protocol</div>
                                </div>
                                <div className="bg-white border-2 border-gray-200 border-l-[12px] border-l-gic-gold p-10 rounded-2xl shadow-sm space-y-8 relative overflow-hidden">
                                    ...
                                    <div className="relative z-10 flex items-center justify-between">
                                        <div className="inline-flex items-center gap-3 px-4 py-2 bg-gray-900 text-white rounded-lg">
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-widest">{recommendation.timing} INTERVENTION PROTOCOL</span>
                                        </div>
                                        <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Authorized Executive Briefing</div>
                                    </div>
                                    <div className="relative z-10 space-y-6">
                                        ...
                                        <div className="grid grid-cols-2 gap-8 pt-6 border-t border-gray-100">
                                            <div>
                                                <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Intervention Nodes</div>
                                                <div className="flex flex-wrap gap-2">
                                                    {recommendation.priorityCommunities.map((c: string) => (
                                                        <span key={c} className="px-3 py-1 bg-gray-50 rounded-lg text-xs font-black text-gray-700 border border-gray-200 uppercase">{c}</span>
                                                    ))}
                                                </div>
                                            </div>
                                            <div>
                                                <div className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Strategic Framing</div>
                                                <p className="text-sm font-bold text-gray-600 italic leading-relaxed">"{recommendation.framing}"</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
