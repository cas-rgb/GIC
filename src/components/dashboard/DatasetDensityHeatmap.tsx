...
const CATEGORIES = [
  ...
  { label: "Public Signals", value: "osint" },
  { label: "Social Media", value: "social" },
  { label: "News Media", value: "news" },
];

export default function DatasetDensityHeatmap({
  data,
  externalMunicipality,
  externalWard,
}: DatasetDensityHeatmapProps) {
    ...
    const filtered = data.filter(
      (item) =>
        (activeCategory === "all" ||
          ...
          item.sourceType?.toLowerCase() === activeCategory) &&
        (activeMunicipality === "all" ||
          item.municipality === activeMunicipality) &&
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-[10px] text-slate-400 uppercase tracking-widest font-black mt-1">
            Regional Intelligence Density (40+ Sources)
          </p>
        </div>
        ...
        {densityData.length > 0 ? (
            ...
            {densityData.slice(0, 10).map((region, index) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="text-[9px] font-black text-slate-400 uppercase tracking-tight truncate max-w-[80%]">
                    {region.name}
                  </span>
                  <Zap
                    className={`w-3 h-3 ${region.intensity > 0.7 ? "text-amber-500" : "text-slate-300"}`}
                  />
                </div>
          ...
          <div className="text-center">
            ...
            <h4 className="text-slate-900 font-bold mb-1">
              No Density Nodes Detected
            </h4>
            <p className="text-[10px] text-slate-400 uppercase tracking-widest font-black">
              Adjust category filters for real-time detection
            </p>
          </div>
        )}
      </div>
      ...
      <div className="mt-8 pt-6 border-t border-slate-100 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <p className="text-[10px] font-bold text-slate-500 italic">
            Across {densityData.length} strategic clusters
          </p>
        </div>
        <button className="text-[10px] font-black text-gic-blue uppercase tracking-widest hover:underline">
          View All Anchors
        </button>
      </div>
    </div>
