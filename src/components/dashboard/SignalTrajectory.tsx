...
export default function SignalTrajectory({ data }: SignalTrajectoryProps) {
  ...
  const filtered = data.filter(
    (sig) =>
      (activeDomain === "all" ||
        sig.domain?.toLowerCase() === activeDomain.toLowerCase()) &&
      (activeMunicipality === "all" ||
        sig.municipality === activeMunicipality) &&
      ...
      <div className="mb-6 flex justify-between items-start">
        <div>
          ...
          <p className="text-[10px] text-slate-400 uppercase tracking-widest font-black">
            7-Day Institutional Risk Momentum
          </p>
        </div>
        ...
        {chartData.length > 0 ? (
          ...
          <div className="text-center">
            ...
            <p className="text-[8px] text-slate-500 uppercase tracking-widest font-black">
              Zero signal movement in selected window
            </p>
          </div>
        )}
      </div>
