...
export default function BriefingLayout({ country, province, metrics, projects, sos, dashboardOneData }: BriefingLayoutProps) {
    ...
    return (
        <div className="hidden print:block min-h-screen bg-white p-12 text-slate-900 font-sans">
            ...
            <div className="flex justify-between items-start border-b-2 border-slate-900 pb-8 mb-12">
                <div>
                    <h1 className="text-4xl font-black uppercase tracking-tighter mb-2">Strategic Intelligence Briefing</h1>
                    <div className="flex items-center gap-4 text-slate-500 font-bold uppercase tracking-widest text-xs">
                        <Flag className="w-4 h-4" />
                        <span>{country} / {province} Regional Profile</span>
                    </div>
                </div>
            ...
            {dashboardOneData && (
                <div className="mb-12 space-y-12">
                    <section>
                        <h2 className="text-2xl font-black uppercase tracking-tight mb-6 border-l-4 border-slate-900 pl-4">Executive Summary: {province}</h2>
                        <div className="p-8 bg-slate-50 rounded-3xl border border-slate-100 italic text-sm leading-relaxed text-slate-600">
                            The following synthesis is generated from {dashboardOneData.kpis.concernVolume.value || 0} validated provincial signals. 
                            The top public concern is currently identified as <span className="text-slate-900 font-black uppercase">{dashboardOneData.kpis.topConcern.value || 'N/A'}</span>.
                            Strategic alignment with provincial budget priorities is rated as <span className="text-slate-900 font-black uppercase">{dashboardOneData.kpis.alignmentScore.rating}</span>.
                        </div>
                    </section>
                            ...
                            <div className="space-y-4">
                                {dashboardOneData.breakdown.slice(0, 5).map((b, i) => (
                                    <div key={i} className="flex justify-between items-center py-2 border-b border-slate-100">
                                        <span className="text-sm font-bold text-slate-700">{b.topic}</span>
                                        <span className="text-xs font-black text-slate-400">{b.percentage.toFixed(1)}% SOV</span>
                                    </div>
                                ))}
                            </div>
                        </section>
                            ...
                            <div className="space-y-4">
                                {dashboardOneData.distribution.slice(0, 5).map((d, i) => (
                                    <div key={i} className="flex justify-between items-center py-2 border-b border-slate-100">
                                        ...
                                        <span className="text-xs font-black text-slate-900">{d.value} Signals</span>
                                    </div>
                                ))}
                            </div>
                        </section>
                    ...
                    <section>
                        ...
                        <p className="text-sm text-slate-600 leading-relaxed">
                            {dashboardOneData.kpis.alignmentScore.governanceNote}. 
                            Alignment analysis indicates a focus on {province} infrastructure delivery nodes.
                        </p>
                    </section>
                </div>
            )}
            ...
            <div className="fixed bottom-0 left-0 right-0 p-12 bg-white print:static print:mt-12">
                <div className="grid grid-cols-2 gap-12 border-t border-slate-100 pt-8 mb-8">
                    <div className="space-y-4">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Autonomous Data Lineage</h4>
                        <div className="text-[9px] font-bold text-slate-500 uppercase">
                             Governed Reporting Protocol: Next-Gen Civic Intelligence
                        </div>
                    </div>
                    <div className="text-right">
                        <h4 className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2">Institutional Verification</h4>
