...
export default function ExecutiveBriefingCenter({
  province,
  signals,
}: ExecutiveBriefingCenterProps) {
  const [isGenerating, setIsGenerating] = useState(false);
  const [memo, setMemo] = useState<ExecutiveMemo | null>(null);
  const memoReference = memo
    ? buildReferenceCode("REF", `${province}-${memo.title}-${signals.length}`)
    : null;
  const sourceReference = memo
    ? buildReferenceCode(
        "SRC",
        `${province}-${memo.timestamp}-${signals.length}`,
      )
    : null;
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              Autonomous Narrative Inception
            </p>
          </div>
        </div>
        ...
        <AnimatePresence mode="wait">
          {!memo && !isGenerating ? (
            ...
            >
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-slate-500 font-medium">
                  Click generate to trigger the AI Narrative Inception pipeline.
                  The Intelligence Core will compile current regional signals and deliver a
                  calibrated strategy memo.
                </p>
              </div>
            </motion.div>
          ) : isGenerating ? (
            ...
            >
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gic-blue animate-pulse">
              ...
              <div className="relative z-10 flex-1 flex flex-col">
                ...
                <div className="space-y-10 flex-1">
                  <div className="space-y-4">
                    <h4 className="text-xs font-black text-slate-900 uppercase tracking-widest border-l-4 border-gic-blue pl-4">
                      Executive Summary
                    </h4>
                    ...
                    <ul className="space-y-4">
                      {memo.findings.map((finding, i) => (
                        <li key={i} className="flex gap-4">
                          <span className="text-gic-blue font-black flex-shrink-0">
                            0{i + 1}.
                          </span>
                    ...
                    <div className="bg-slate-50 p-4 rounded-xl border border-slate-100 max-w-sm">
                      ...
                      <div className="space-y-1">
                        ...
                        <div className="flex justify-between text-[8px] font-bold text-slate-500 uppercase">
                          ...
                          <span>Verified Public Signal</span>
                        </div>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
                      ...
                      <div className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                        Verified Report Statement
                        <br />
                        Traceable to Source ID: {sourceReference}
                      </div>
                    </div>
                  </div>
