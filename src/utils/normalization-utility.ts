...
export class NormalizationUtility {
  ...
  static normalizeRegion(province: string, municipality: string) {
    const provMatch = SA_REGIONAL_REGISTRY.provinces.find(
      (p) =>
    ...
    const muniMatch = provMatch.municipalities.find(
      (m) =>
  ...
  static normalizeSector(input: string): Category {
    ...
    if (
      ...
      return "Planning";
    if (lower.includes("civil") || lower.includes("infrastructure"))
      return "Civil";
  ...
  static normalizeSource(input: string): string {
    ...
    if (lower.includes("wikipedia") || lower.includes("wiki"))
      return "Wikipedia";
    if (lower.includes("gic") || lower.includes("internal"))
      return "GIC Internal";
