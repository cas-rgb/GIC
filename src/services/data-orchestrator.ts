...
import { LeaderEntry, TenderNotice, InfrastructureOpportunity, RiskSignalEntry, PlanningBudget } from "@/types/database";
...
export class GlobalDataOrchestrator {
     ...
     */
    static async performRegionalDeepDive(country: 'South Africa' | 'Namibia' | 'Botswana' | 'Eswatini' | 'Lesotho', province: string, municipality: string, ward?: string) {
        const { GIC_QUERY_LIBRARY } = await import("@/data/query-library");
        const wardDisplay = ward ? (ward.toLowerCase().startsWith('ward') ? ` (${ward})` : ` (Ward ${ward})`) : '';
        ...
        if (!ward) { // Only for municipality level
            const wikiData = await GlobalDataOrchestrator.fetchWikipediaBaseline(country, province, municipality);
            results.push(...wikiData);
        }

        // 1. Execute Thematic Search Waves
        for (const queryNode of queries) {
            console.log(`  > Wave: ${queryNode.theme} | Priority: ${queryNode.priority}`);
            
            // Inject context into query
            const contextSearch = `${queryNode.query} ${municipality} ${ward ? `${ward} ` : ''}${province}`.trim();
            ...
            try {
                ...
                if (search.results && search.results.length > 0) {
                    ...
                    console.log(`    - Found ${search.results.length} intelligence signals.`);
                }
        ...
        if (GIC_QUERY_LIBRARY['Regional']) {
            ...
            for (const rNode of GIC_QUERY_LIBRARY['Regional']) {
                try {
                    const rSearch = await tavilyClient.search(`${rNode.query} ${country}`, { searchDepth: "advanced", maxResults: 3 });
                    results.push({
                        id: crypto.randomUUID(),
        ...
        await this.persistToUniversalFeed(results, country, province, municipality, ward);

        // 4. Backward Compatibility: AI Transformation & Dataset Entries
        try {
            await this.transformAndPersist(results, country, province, municipality, ward);
        } catch (error) {
     ...
     */
    private static async persistToUniversalFeed(entries: DatasetEntry[], country: string, province: string, municipality: string, ward?: string) {
        ...
        for (const theme of themes) {
            ...
            try {
                ...
                console.log(`  [UNIVERSAL] Persisted ${combinedPayload.length} signals for theme: ${theme}`);
            } catch (error) {
                console.error(`  [UNIVERSAL ERROR] Failed to persist theme ${theme}:`, error);
            }
        }
    }

    private static async fetchWikipediaBaseline(country: 'South Africa' | 'Namibia' | 'Botswana' | 'Eswatini' | 'Lesotho', province: string, municipality: string): Promise<DatasetEntry[]> {
    ...
    private static async transformAndPersist(rawEntries: DatasetEntry[], country: string, province: string, municipality: string, ward?: string) {
        console.log(`Transforming raw signals for ${municipality}${ward ? ` (Ward ${ward})` : ''}...`);
        
        const combinedContent = rawEntries.map(e => JSON.stringify(e.payload)).join("\n\n");
        ...
        console.log(`[AI RESPONSE RAW] for ${municipality}:`, responseText.substring(0, 500));
        
        const structuredData = extractJsonObject(responseText);
        console.log(`[STRUCTURED DATA] for ${municipality}:`, structuredData ? "SUCCESS" : "FAILED_PARSING");
        if (structuredData) {
            ...
            if (structuredData.leaders) await this.saveToCollection("leaders", structuredData.leaders, country, province, municipality, ward);
            if (structuredData.tenders) await this.saveToCollection("tenders", structuredData.tenders, country, province, municipality, ward);
            if (structuredData.leaders) await this.saveToCollection("leaders", structuredData.leaders, country, province, municipality, ward);
            if (structuredData.tenders) await this.saveToCollection("tenders", structuredData.tenders, country, province, municipality, ward);
            if (structuredData.opportunities) await this.saveToCollection("opportunities", structuredData.opportunities, country, province, municipality, ward);
            if (structuredData.risks) await this.saveToCollection("riskSignals", structuredData.risks, country, province, municipality, ward);
            if (structuredData.planning) await this.saveToCollection("planningBudgets", structuredData.planning, country, province, municipality, ward);
            if (structuredData.signals) await this.saveToCollection("riskSignals", structuredData.signals, country, province, municipality, ward);
        }
    }

    private static async saveToCollection(colName: string, items: any[], country: string, province: string, municipality: string, ward?: string) {
        const collectionRef = collection(db, colName);
        for (const item of items) {
            try {
                // Normalize Geography and Sector
                const { province: normProv, municipality: normMuni } = NormalizationUtility.normalizeRegion(province, municipality);
                const normSector = item.category ? NormalizationUtility.normalizeSector(item.category) : undefined;
                const normDomain = item.domain ? NormalizationUtility.normalizeDomain(item.domain) : undefined;
                const normSource = item.source_type ? NormalizationUtility.normalizeSource(item.source_type) : undefined;
    ...
    private static async persistDatasetEntries(entries: DatasetEntry[]) {
        ...
        for (const entry of entries) {
            try {
                ...
                console.error(`[SAVE ERROR] in strategicDatasets for ${entry.municipality}:`, error);
            }
        }
    }
