...
export class CommunityIntelligenceSyndicator {
  ...
  static async syndicateAll() {
    ...
    const areas = Array.from(
      new Set(signals.map((s) => s.municipality || s.province || "Unknown")),
    );
    console.log(
      `🔍 Found ${signals.length} signals across ${areas.length} areas.`,
    );

    for (const area of areas) {
      await this.syndicateArea(
        area,
        signals.filter((s) => (s.municipality || s.province) === area),
      );
    }
  }
  ...
  static async syndicateArea(area: string, areaSignals: any[]) {
    ...
    try {
      ...
      const signalText = prioritizedSignals
        .map((s) => `[${s.category}] ${s.text} (Urgency: ${s.urgency})`)
        .join("\n");
