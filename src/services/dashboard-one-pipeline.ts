...
import { CommunityIssue, BudgetReference, ProvincialBudgetAllocation, LocationResolvedSignal } from '@/types/dashboard-one';
...
export class DashboardOnePipeline {
    ...
    static async retrieveProvincialSignals(province: string) {
        ...
        for (const res of allResults) {
            ...
            await setDoc(doc(collection(db, "public_signal_raw"), id), {
                ...res,
                source_type: res.url.includes('gov.za') ? 'government_reference_raw' : 'news_article',
    ...
    static async classifySignal(signal: any) {
        ...
        if (classification) {
            ...
            if (classification.municipality) {
                ...
                await setDoc(doc(collection(db, "location_resolved_signal"), geoId), geoSignal);
            }
        }
    }
    ...
    static async extractBudgetSignals(signal: any) {
        if (!signal.url.includes('budget') && !signal.title.toLowerCase().includes('budget')) return;
        ...
        for (const alloc of allocations) {
            const id = `budget-${signal.province}-${alloc.budget_topic}-${alloc.fiscal_year}`.replace(/\s+/g, '-').toLowerCase();
            ...
            await setDoc(doc(collection(db, "provincial_budget_topic_allocation"), id), budgetAlloc);
        }
    }
