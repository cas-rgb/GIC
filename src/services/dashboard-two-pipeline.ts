...
import { ServicePressureCase, ServiceDomain, PressureType } from '@/types/dashboard2';

export class DashboardTwoPipeline {
  static SERVICE_DOMAINS: ServiceDomain[] = [
    'Water Infrastructure', 'Electricity Supply', 'Waste Management',
    'Sanitation', 'Roads and Transport', 'Public Transport',
    'Healthcare', 'Housing and Settlements', 'Local Governance',
    'Community Safety', 'Provincial Infrastructure', 'Other'
  ];

  static PRESSURE_TYPES: PressureType[] = [
    'Outage', 'Delay', 'Breakdown', 'Backlog', 'Complaint',
    'Protest', 'Governance Failure', 'Repair / Response',
    'Infrastructure Deterioration', 'Access Failure', 'Other'
  ];
  ...
  static async retrievePressureSignals(province: string) {
    ...
    for (const query of queries) {
      ...
      if (results?.results) {
        allResults.push(...results.results.map((r: any) => ({ ...r, province, query })));
      }
    }
   ...
   */
  static async extractPressureCase(signal: any): Promise<ServicePressureCase | null> {
    const prompt = `
    ...
    try {
      ...
      if (data) {
        ...
        await setDoc(doc(collection(db, "service_pressure_case"), id), pressureCase);
        return pressureCase;
      }
    } catch (error) {
