  ...
  calculateSOS: async (
    ...
    try {
      ...
      const riskSeverity =
        risks.size > 0
          ? risks.docs.reduce(
              (acc, doc) => acc + (doc.data().severity || 5),
              0,
            ) / risks.size
          : 0;
      ...
      const weightedPositive =
        momentumScore * 0.3 + leadershipScore * 0.2 + alignmentScore * 0.3;
        ...
        trace: [
          {
            table: "tenders",
            query: baseQuery,
            sourceCount: tenders.size,
            timestamp,
          },
          {
            table: "leaders",
            query: baseQuery,
            sourceCount: leaders.size,
            timestamp,
          },
          {
            table: "planningBudgets",
            query: baseQuery,
            sourceCount: planning.size,
            timestamp,
          },
          {
            table: "riskSignals",
            query: baseQuery,
            sourceCount: risks.size,
            timestamp,
          },
        ],
      };
