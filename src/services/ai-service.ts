import { AISignal, PredictionData, EngagementStrategy, NarrativeDriver, RiskIndicator, StrategicRecommendation } from "@/types";
...
export const geminiPro = genAI.getGenerativeModel({ model: "gemini-3.1-pro-preview" }); 
...
export async function analyzeCommunitySentiment(content: string, context?: string): Promise<AISignal[]> {
    const prompt = `
...
export async function analyzeNarrativeAmplification(content: string): Promise<NarrativeDriver[]> {
...
export async function getPredictiveRiskScore(signals: AISignal[]): Promise<RiskIndicator[]> {
...
export async function generateStrategicRecommendations(risks: RiskIndicator[], province: string): Promise<StrategicRecommendation> {
    ...
    try {
        ...
        return data || { timing: 'monitoring', framing: 'Standard oversight', priorityCommunities: [], mitigationStrategy: 'Maintain baseline monitoring' };
    } catch (error) {
        console.error("Recommendation Error:", error);
        return { timing: 'monitoring', framing: 'Neutral community engagement', priorityCommunities: [], mitigationStrategy: 'Continue baseline monitoring' };
    }
}

export async function forecastInfraDemand(community: string, category: string, history: string): Promise<PredictionData> {
    ...
    try {
        const result = await geminiFlash.generateContent(prompt);
        const data = extractJsonObject(result.response.text());
        return data || { forecast6mo: 0, forecast12mo: 0, forecast36mo: 0, confidence: 0.5 };
    } catch (error) {
        console.error("Forecast Error:", error);
        return { forecast6mo: 2, forecast12mo: 5, forecast36mo: 12, confidence: 0.3 };
    }
}

export async function generateEngagementStrategy(community: string, signals: string): Promise<EngagementStrategy[]> {
