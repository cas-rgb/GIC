...
import { GovernedMetric, METRIC_REGISTRY, getConfidenceRating, calculateConfidence } from '@/lib/reporting-schema';
import { CommunityIssue, ProvincialBudgetAllocation } from '@/types/dashboard-one';
...
export const DashboardOneAggregator = {
    ...
    getExecutiveKPIs: async (province: string) => {
        ...
        const qIssues = query(issueRef, where('province', '==', province), where('citizen_concern_indicator', '==', true));
        ...
        const budgets = budgetSnap.docs.map(d => d.data() as ProvincialBudgetAllocation);

        // A. Public Concern Volume
        const concernVolumeScore = calculateConfidence([{ count: issues.length, expected: 50 }]);
        const concernVolume: GovernedMetric<number> = {
            ...
            trace: [{ table: 'community_issue', query: `province=${province}`, sourceCount: issues.length, timestamp }]
        };

        // B. Top Public Concern
        const topicCounts = issues.reduce((acc, curr) => {
            acc[curr.primary_topic] = (acc[curr.primary_topic] || 0) + 1;
            return acc;
        }, {} as Record<string, number>);

        const topTopic = Object.entries(topicCounts).sort((a, b) => b[1] - a[1])[0];
        const topConcernScore = calculateConfidence([{ count: issues.length, expected: 30 }]);
        const topConcern: GovernedMetric<string> = {
            ...
            trace: [{ table: 'community_issue', query: `province=${province} group by topic`, sourceCount: issues.length, timestamp }]
        };
        ...
        if (budgets.length > 0) {
            ...
            method = budgets.some(b => b.allocation_amount !== null) ? 'numeric' : 'qualitative';
        }

        const alignmentScoreVal = calculateConfidence([{ count: budgets.length, expected: 5 }]);
        const alignmentScore: GovernedMetric<any> = {
            ...
            governanceNote: budgets.length === 0 ? 'Provincial budget dataset required' : (method === 'qualitative' ? 'Based on strategic priorities' : 'Based on budget allocations'),
            trace: [{ table: 'provincial_budget_topic_allocation', query: `province=${province}`, sourceCount: budgets.length, timestamp }]
        };

        return serializeData({ concernVolume, topConcern, concernVelocity, alignmentScore });
    },
    ...
    getTopicBreakdown: async (province: string) => {
        const issuesSnap = await getDocs(query(collection(db, "community_issue"), where('province', '==', province)));
        const issues = issuesSnap.docs.map(d => d.data() as CommunityIssue);
        const total = issues.length;
        if (total === 0) return [];

        const topicMap = issues.reduce((acc, curr) => {
            acc[curr.primary_topic] = (acc[curr.primary_topic] || 0) + 1;
            return acc;
        }, {} as Record<string, number>);

        const results = Object.entries(topicMap).map(([topic, count]) => ({
            topic,
            mentions: count,
    ...
    getMunicipalityDistribution: async (province: string) => {
        const geoSnap = await getDocs(query(collection(db, "location_resolved_signal"), where('province', '==', province)));
        const geoSignals = geoSnap.docs.map(d => d.data() as any);

        const muniMap = geoSignals.reduce((acc, curr) => {
            const muni = curr.municipality || 'Unknown';
            acc[muni] = (acc[muni] || 0) + 1;
            return acc;
        }, {} as Record<string, number>);

        const results = Object.entries(muniMap).map(([name, value]) => ({
            name,
            value
    ...
    getAlignmentMatrix: async (province: string) => {
        ...
        const budgetSnap = await getDocs(query(collection(db, "provincial_budget_topic_allocation"), where('province', '==', province)));
        const budgets = budgetSnap.docs.map(d => d.data() as ProvincialBudgetAllocation);

        const results = breakdown.map(b => {
            const budget = budgets.find(bg => bg.budget_topic.toLowerCase().includes(b.topic.toLowerCase()));
            return {
                topic: b.topic,
