...
export const IntelligenceEngine = {
  ...
  getExecutiveBrief: async (
    ...
    const weightedSeverity =
      signals.length > 0
        ? signals.reduce((acc, s) => acc + (s.severity || 5), 0) /
          signals.length
        : 0;
    ...
    return {
      ...
      recommendation:
        weightedSeverity > 4
          ? "CRITICAL: Deploy regional intervention unit."
          : "Standard oversight active.",
      confidence: 0.94,
  ...
  getInfrastructureSynergy: async (country: string, province: string) => {
    ...
    tenders.docs.forEach((doc) => {
      ...
      if (!muniMap[data.municipality].includes(data.sector))
        muniMap[data.municipality].push(data.sector);
    });
  ...
  getSentimentRiskCorrelation: async (municipality: string) => {
    ...
    const avgSeverity =
      risks.reduce((acc, r) => acc + (r.severity || 0), 0) /
      (risks.length || 1);
    ...
    return {
      ...
      impact:
        avgSeverity > 6
          ? "High probability of social disruption to project timelines."
          : "Social stability supports current milestones.",
  ...
  getProjectVulnerabilitySynthesis: async (projectId: string) => {
    try {
      ...
      const signalsSnap = await getDocs(
        query(collection(db, "riskSignals"), ...filters, limit(10)),
      );

      const risks = signalsSnap.docs.map((d) => d.data());
      const avgSeverity =
        risks.length > 0
          ? risks.reduce((acc, r: any) => acc + (r.severity || 5), 0) /
            risks.length
          : 0;
      ...
      return {
        ...
        riskLevel:
          avgSeverity > 7 ? "Extreme" : avgSeverity > 4 ? "High" : "Nominal",
        recommendation:
          avgSeverity > 6
            ? "Immediate site audit recommended."
  ...
  getBoardroomPortfolioSummary: async () => {
    ...
    const totalCapex = internalSnap.docs.reduce(
      (acc, d) => acc + (d.data().budget || 0),
      0,
    );
    const avgProgress =
      internalSnap.docs.reduce((acc, d) => acc + (d.data().progress || 0), 0) /
      (internalSnap.size || 1);
  ...
  getRegionalHeatmapData: async (province: string) => {
    ...
    const clusters: Record<
      string,
      { count: number; intensity: number; threats: string[] }
    > = {};
    ...
    snap.docs.forEach((doc) => {
      ...
      if (!clusters[muni])
        clusters[muni] = { count: 0, intensity: 0, threats: [] };
