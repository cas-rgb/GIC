...
export function getPressureBreakdownConfig(
  data: ProvinceServicePressureBreakdownRow[],
) {
  return {
    ...
    series: [
      {
        ...
        data: data.map((d) => ({
          ...
          itemStyle: {
            color:
              d.serviceDomain === "Water Infrastructure"
                ? "#0ea5e9"
                : d.serviceDomain === "Electricity Supply"
                  ? "#f59e0b"
                  : d.serviceDomain === "Roads and Transport"
                    ? "#6366f1"
                    : d.serviceDomain === "Healthcare"
                      ? "#ec4899"
                      : d.serviceDomain === "Waste Management"
                        ? "#10b981"
                        : "#94a3b8",
          },
        })),
...
export function getPressureTrendConfig(
  data: ProvinceServicePressureTrendRow[],
) {
  ...
  return {
    ...
    series: domains.map((domain) => ({
      ...
      data: dates.map((date) => {
        const row = data.find(
          (d) => d.reportingDate === date && d.serviceDomain === domain,
        );
...
export function getExposureRankingConfig(
  data: ProvinceMunicipalityExposureRow[],
) {
...
export function getStressMatrixConfig(
  data: ProvinceStressEscalationMatrixRow[],
) {
  return {
    tooltip: {
      trigger: "item",
      formatter: (params: any) => {
        const d = params.data;
        if (!d || !d.value || d.value.length < 2) return "";
    ...
    series: [
      {
        type: "scatter",
        symbolSize: (data: any) => {
          if (!data || data.length < 3) return 10;
          return Math.max(10, data[2] * 3);
        },
        data: data.map((d) => ({
          name: d.municipality,
          value: [
            d.currentPressureScore || 0,
            d.recentEscalationScore || 0,
            d.severityWeight || 1,
          ],
          status: d.stressStatus,
          itemStyle: {
            color:
              d.stressStatus === "Acute Flashpoint"
                ? "#e11d48"
                : d.stressStatus === "Escalating Risk"
                  ? "#f59e0b"
                  : d.stressStatus === "Chronic Pressure"
                    ? "#3b82f6"
                    : "#94a3b8",
            opacity: 0.8,
            shadowBlur: 10,
