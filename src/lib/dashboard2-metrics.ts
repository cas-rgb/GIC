  ...
  return {
    ...
    value: {
      ...
      shareOfPressurePercentage: Number(
        ((domainCount / filtered.length) * 100).toFixed(1),
      ),
    },
...
export function buildServicePressureTrajectory(
  ...
  value: number;
  direction: "rising" | "stable" | "easing";
}> {
  ...
  const current =
    counts.slice(-3).reduce((a, b) => a + b, 0) / Math.min(3, counts.length);
  const baseline =
    counts.slice(0, -3).reduce((a, b) => a + b, 0) /
    Math.max(1, counts.length - 3);
