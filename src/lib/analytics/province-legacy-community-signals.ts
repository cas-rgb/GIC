...
export async function getProvinceLegacyCommunitySignals(
  ...
  return {
    ...
    caveats: [
      "This panel uses legacy Firebase community signals that were imported into the governed document layer.",
      "Legacy community signals strengthen resident and civic issue visibility, but they are less standardized than current official and live-ingested sources.",
    ],
    trace: {
      tables: ["documents", "locations", "sources"],
      query: `province=${province ?? "all"};parser=legacy-community-signals-v1;days=${days}`,
    },
  };
}
