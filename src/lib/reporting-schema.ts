...
export const METRIC_REGISTRY = {
  SOS: {
    ...
    description: 'Weighted index of infrastructure momentum, stability, and risk.'
  },
  PORTFOLIO_DENSITY: {
  ...
  RISK_INTENSITY: {
    ...
    description: 'Aggregated severity of detected environmental and social risks.'
  },
  DELIVERY_CONFIDENCE: {
    id: 'gic.executive.delivery',
    label: 'National Delivery Confidence',
    description: 'Calculated reliability of project timelines and budget alignment.'
  },
  ...
  GOVERNMENT_ALIGNMENT: {
    ...
    description: 'Comparison of citizen concern frequency vs provincial budget allocation.'
  },
  ...
  MOST_EXPOSED_MUNI: {
    ...
    description: 'Municipality with the highest concentration of service strain.'
  },
  PRESSURE_BREAKDOWN: {
...
export function calculateConfidence(sources: { count: number, expected: number }[]): number {
  if (sources.length === 0) return 0;
  const totalWeight = sources.length;
  const sum = sources.reduce((acc, s) => acc + Math.min(s.count / s.expected, 1.0), 0);
