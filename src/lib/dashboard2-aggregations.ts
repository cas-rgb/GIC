...
export function buildProvinceServicePressureBreakdown(
  ...
  for (const row of filtered) {
    const current = grouped.get(row.serviceDomain) || {
      count: 0,
      highSeverityCount: 0,
    };
    current.count += 1;
  ...
  return Array.from(grouped.entries())
    .map(([serviceDomain, data]) => ({
      province,
      serviceDomain:
        serviceDomain as ProvinceServicePressureBreakdownRow["serviceDomain"],
      pressureCaseCount: data.count,
      shareOfPressurePercentage: Number(
        ((data.count / Math.max(total, 1)) * 100).toFixed(1),
      ),
...
export function buildProvinceServicePressureTrend(
  ...
  const records = Array.from(grouped.entries()).map(([key, data]) => {
    ...
    return {
      ...
      reportingDate,
      serviceDomain:
        serviceDomain as ProvinceServicePressureTrendRow["serviceDomain"],
      pressureCaseCount: data.count,
      rolling7DayAverage: null,
      trendDirection: "UNKNOWN" as const,
      highSeverityShare: Number(
        ((data.highSeverityCount / data.count) * 100).toFixed(1),
      ),
      confidence: toConfidence(data.count),
      sourceCount: data.count,
      trace: [
...
export function buildProvinceMunicipalityExposure(
  ...
  return Array.from(grouped.entries())
    .map(([municipality, data]) => {
      const entries = Array.from(data.domainCounts.entries()).sort(
        (a, b) => b[1] - a[1],
      );
...
export function buildProvinceStressEscalationMatrix(
  ...
  return municipalityRows.map((row) => {
    ...
    let stressStatus: ProvinceStressEscalationMatrixRow["stressStatus"] =
      "Monitored";
    if (currentPressureScore >= 10 && recentEscalationScore >= 6)
      stressStatus = "Acute Flashpoint";
