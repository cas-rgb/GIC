...
export class IngestionValidator {
    ...
    static tagGeography(content: string): string[] {
        const regions = ['Gauteng', 'KZN', 'Western Cape', 'Limpopo', 'Maseru', 'Mbabane'];
        return regions.filter(region => content.toLowerCase().includes(region.toLowerCase()));
    }

    /**
     * Simulates duplicate detection.
     */
    static detectDuplicates(record: Partial<IngestionRecord>, existingRecords: IngestionRecord[]): string | undefined {
     ...
     */
    static classifySeverity(content: string): 'critical' | 'high' | 'medium' | 'low' {
