...
import { CommunitySignal, EvidenceSource, SignalLayer } from '../types/database';
...
export const DataFetcher = {
     ...
     */
    async discoverSignals(queryContext: string = "Gauteng Infrastructure"): Promise<{ signals: CommunitySignal[], evidence: EvidenceSource[] }> {
        try {
            ...
            searchResult.results.forEach((res, index) => {
                ...
                const signal: CommunitySignal = {
                    ...
                    status: 'active',
                    reliabilityScore: IntelligenceEngine.CREDIBILITY_WEIGHTS[sourceType] || 0.5,
    ...
    classifySource(url: string): CommunitySignal['sourceType'] {
        if (url.includes('news24') || url.includes('sabc') || url.includes('iol')) return 'local_news';
        if (url.includes('dailymaverick') || url.includes('groundup')) return 'journalist';
    ...
    detectLayers(text: string): SignalLayer[] {
        ...
        if (lowText.match(/water|pothole|sewage|electricity|power|road/)) layers.push('issue');
        if (lowText.match(/protest|strike|march|shutdown/)) layers.push('event', 'risk');
        if (lowText.match(/frustrated|angry|outrage|hope|pride/)) layers.push('sentiment');
        if (lowText.match(/trend|growing|rapidly|velocity/)) layers.push('momentum');
        if (lowText.match(/expansion|new housing|development/)) layers.push('development');
    ...
    detectSentiment(text: string): 'positive' | 'negative' | 'neutral' {
        const lowText = text.toLowerCase();
        if (lowText.match(/great|success|improving|restored|fixed/)) return 'positive';
        if (lowText.match(/fail|outage|shortage|angry|worst|broken/)) return 'negative';
        return 'neutral';
    },

    detectEmotion(text: string): CommunitySignal['emotion'] {
        const lowText = text.toLowerCase();
        if (lowText.includes('angry') || lowText.includes('outrage')) return 'anger';
