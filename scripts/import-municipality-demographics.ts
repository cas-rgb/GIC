...
async function main() {
  const rows = await readCsvRows("data/reference/demographics/municipality_demographics_yearly.csv");
  let upserted = 0;
  ...
  for (const row of rows) {
    ...
    if (!provinceName || !municipalityName || year === null) {
      continue;
    }

    await query(
      `
        insert into municipality_demographics_yearly (
          municipality_code,
          municipality_name,
          province_code,
          province_name,
        ...
        values ($1,$2,$3,$4,$5,$6,$7,$8,$9::jsonb,$10,$11,$12,$13::jsonb,$14::jsonb,$15::jsonb,$16,$17,now())
        on conflict (province_name, municipality_name, year)
        do update set
  ...
  console.log(JSON.stringify({ file: "municipality_demographics_yearly.csv", rowCount: rows.length, upserted }, null, 2));
}
